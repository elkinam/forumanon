...
$url = new moodle_url('/mod/forumanon/user.php');
if ($course !== SITEID) {
...
add_to_log($course->id, "forumanon", "user report",
        "user.php?course=$course->id&amp;id=$user->id&amp;mode=$mode", "$user->id");

$strforumposts   = get_string('forumposts', 'forumanon');
$strparticipants = get_string('participants');
$strmode         = get_string($mode, 'forumanon');
...
// Get the posts.
$posts = forumanon_search_posts($searchterms, $searchcourse, $page*$perpage, $perpage, $totalcount, $extrasql);
if ($posts) {
    ...
    foreach ($posts as $post) {
        ...
        if (!isset($forums[$discussion->forum])) {
            ...
            $forum->cm = get_coursemodule_from_instance('forumanon', $forum->id, 0, false, MUST_EXIST);
        ...
        $forumurl = new moodle_url('/mod/forumanon/view.php', array('id' => $forum->cm->id));
        $discussionurl = new moodle_url('/mod/forumanon/discuss.php', array('d' => $discussion->id));
        ...
        if ($forum->type != 'single') {
            ...
            if ($post->parent != 0) {
                $parenturl = new moodle_url('/mod/forumanon/discuss.php', array('d' => $post->discussion, 'parent' => $post->id));
        ...
        $fulllink = html_writer::link($discussionurl, get_string("postincontext", "forumanon"));

        forumanon_print_post($post, $discussion, $forum, $forum->cm, $course, false, false, false, $fulllink);
        echo "<br />";
    }
    ...
    if ($mode == 'posts') {
        echo $OUTPUT->heading(get_string('noposts', 'forumanon'));
    } else {
        echo $OUTPUT->heading(get_string('nodiscussionsstartedby', 'forumanon'));
    }
}
echo '</div>';
echo $OUTPUT->footer();
