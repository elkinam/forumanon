...
class forumanon_portfolio_caller extends portfolio_module_caller_base {
    ...
    function __construct($callbackargs) {
        ...
        if (!$this->postid && !$this->discussionid) {
            throw new portfolio_caller_exception('mustprovidediscussionorpost', 'forumanon');
        }
    }
    /**
     * @global object
    ...
    public function load_data() {
        ...
        if ($this->postid) {
            if (!$this->post = $DB->get_record('forumanon_posts', array('id' => $this->postid))) {
                throw new portfolio_caller_exception('invalidpostid', 'forumanon');
            }
        }
        ...
        if (!$this->discussion = $DB->get_record('forumanon_discussions', $dbparams)) {
            throw new portfolio_caller_exception('invaliddiscussionid', 'forumanon');
        }

        if (!$this->forum = $DB->get_record('forumanon', array('id' => $this->discussion->forum))) {
            throw new portfolio_caller_exception('invalidforumid', 'forumanon');
        }

        if (!$this->cm = get_coursemodule_from_instance('forumanon', $this->forum->id)) {
        ...
        if ($this->post) {
            if ($this->attachment) {
                ...
                $attach = $fs->get_area_files($this->modcontext->id, 'mod_forumanon', 'attachment', $this->post->id, 'timemodified', false);
                $embed  = $fs->get_area_files($this->modcontext->id, 'mod_forumanon', 'post', $this->post->id, 'timemodified', false);
            ...
            $fs = get_file_storage();
            $this->posts = forumanon_get_all_discussion_posts($this->discussion->id, 'p.created ASC');
            $this->multifiles = array();
            foreach ($this->posts as $post) {
                $attach = $fs->get_area_files($this->modcontext->id, 'mod_forumanon', 'attachment', $post->id, 'timemodified', false);
                $embed  = $fs->get_area_files($this->modcontext->id, 'mod_forumanon', 'post', $post->id, 'timemodified', false);
                $files = array_merge($attach, $embed);
                if ($files) {
    ...
    function get_return_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/forumanon/discuss.php?d=' . $this->discussion->id;
    }
    /**
     * @global object
     * @return array
    ...
    function get_navigation() {
        ...
        $navlinks[] = array(
            ...
            'link' => $CFG->wwwroot . '/mod/forumanon/discuss.php?d=' . $this->discussion->id,
            'type' => 'title'
        );
        ...
        if ($this->attachment) { // simplest case first - single file attachment
            ...
            if ($writingleap) {
                ...
                $selection = new portfolio_format_leap2a_entry('forumdiscussion' . $this->discussionid,
                    get_string('discussion', 'forumanon') . ': ' . $this->discussion->name, 'selection');
    ...
    /**
     * this is a very cut down version of what is in forumanon_make_mail_post
     *
     * @global object
     * @param int $post
     * @return string
    ...
    private function prepare_post($post, $fileoutputextras=null) {
        ...
        $formattedtext = portfolio_rewrite_pluginfile_urls($formattedtext, $this->modcontext->id, 'mod_forumanon', 'post', $post->id, $format);
        ...
        if (is_array($this->keyedfiles) && array_key_exists($post->id, $this->keyedfiles) && is_array($this->keyedfiles[$post->id]) && count($this->keyedfiles[$post->id]) > 0) {
            ...
            $output .= '<br /><b>' .  get_string('attachments', 'forumanon') . '</b>:<br /><br />';
    ...
    function check_permissions() {
        ...
        if ($this->post) {
            return (has_capability('mod/forumanon:exportpost', $context)
                || ($this->post->userid == $this->user->id
                    && has_capability('mod/forumanon:exportownpost', $context)));
        }
        return has_capability('mod/forumanon:exportdiscussion', $context);
    }
    /**
     * @return string
     */
    public static function display_name() {
        return get_string('modulename', 'forumanon');
    }
