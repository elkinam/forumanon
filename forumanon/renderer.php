 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 **/
class mod_forumanon_renderer extends plugin_renderer_base {
    /**
    ...
    public function subscriber_overview($users, $forum , $course) {
        ...
        if (!$users || !is_array($users) || count($users)===0) {
            $output .= $this->output->heading(get_string("nosubscribers", "forumanon"));
        } else {
            $output .= $this->output->heading(get_string("subscribersto","forumanon", "'".format_string($forum->name)."'"));
            $table = new html_table();
            $table->cellpadding = 5;
            $table->cellspacing = 5;
    ...
    public function subscribed_users(user_selector_base $existingusers) {
        ...
        $output .= html_writer::tag('p', get_string('forcessubscribe', 'forumanon'));
