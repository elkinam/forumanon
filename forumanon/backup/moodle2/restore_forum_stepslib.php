...
class restore_forum_activity_structure_step extends restore_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $paths[] = new restore_path_element('forum', '/activity/forumanon');
        if ($userinfo) {
            $paths[] = new restore_path_element('forum_discussion', '/activity/forumanon/discussions/discussion');
            $paths[] = new restore_path_element('forum_post', '/activity/forumanon/discussions/discussion/posts/post');
            $paths[] = new restore_path_element('forum_rating', '/activity/forumanon/discussions/discussion/posts/post/ratings/rating');
            $paths[] = new restore_path_element('forum_subscription', '/activity/forumanon/subscriptions/subscription');
            $paths[] = new restore_path_element('forum_read', '/activity/forumanon/readposts/read');
            $paths[] = new restore_path_element('forum_track', '/activity/forumanon/trackedprefs/track');
        }
    ...
    protected function process_forum($data) {
        ...
        $newitemid = $DB->insert_record('forumanon', $data);
    ...
    protected function process_forum_discussion($data) {
        ...
        $newitemid = $DB->insert_record('forumanon_discussions', $data);
    ...
    protected function process_forum_post($data) {
        ...
        $newitemid = $DB->insert_record('forumanon_posts', $data);
        $this->set_mapping('forum_post', $oldid, $newitemid, true);

        // If !post->parent, it's the 1st post. Set it in discussion
        if (empty($data->parent)) {
            $DB->set_field('forumanon_discussions', 'firstpost', $newitemid, array('id' => $data->discussion));
        }
    }
    ...
    protected function process_forum_subscription($data) {
        ...
        $newitemid = $DB->insert_record('forumanon_subscriptions', $data);
    }
    ...
    protected function process_forum_read($data) {
        ...
        $newitemid = $DB->insert_record('forumanon_read', $data);
    }
    ...
    protected function process_forum_track($data) {
        ...
        $newitemid = $DB->insert_record('forumanon_track_prefs', $data);
    }
    ...
    protected function after_execute() {
        ...
        $this->add_related_files('mod_forumanon', 'intro', null);
        ...
        $forumrec = $DB->get_record('forumanon', array('id' => $forumid));
        if ($forumrec->type == 'single' && !$DB->record_exists('forumanon_discussions', array('forum' => $forumid))) {
            ...
            $sd->mailnow  = false;
            $sdid = forumanon_add_discussion($sd, null, $sillybyrefvar, $this->task->get_userid());
            // Mark the post as mailed
            $DB->set_field ('forumanon_posts','mailed', '1', array('discussion' => $sdid));
            // Copy all the files from mod_foum/intro to mod_forum/post
            $fs = get_file_storage();
            $files = $fs->get_area_files($this->task->get_contextid(), 'mod_forumanon', 'intro');
            foreach ($files as $file) {
                $newfilerecord = new stdclass();
                $newfilerecord->filearea = 'post';
                $newfilerecord->itemid   = $DB->get_field('forumanon_discussions', 'firstpost', array('id' => $sdid));
                $fs->create_file_from_storedfile($newfilerecord, $file);
            }
        }

        // Add post related files, matching by itemname = 'forum_post'
        $this->add_related_files('mod_forumanon', 'post', 'forum_post');
        $this->add_related_files('mod_forumanon', 'attachment', 'forum_post');
    }
}
