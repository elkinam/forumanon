...
$url = new moodle_url('/mod/forumanon/subscribers.php', array('id'=>$id));
if ($group !== 0) {
...
if (!has_capability('mod/forumanon:viewsubscribers', $context)) {
    print_error('nopermissiontosubscribe', 'forumanon');
}

unset($SESSION->fromdiscussion);

add_to_log($course->id, "forumanon", "view subscribers", "subscribers.php?id=$forum->id", $forum->id, $cm->id);
...
if (data_submitted()) {
    ...
    if ($subscribe) {
        ...
        foreach ($users as $user) {
            if (!forumanon_subscribe($user->id, $id)) {
                print_error('cannotaddsubscriber', 'forumanon', '', $user->id);
            }
        }
    } else if ($unsubscribe) {
        $users = $existingselector->get_selected_users();
        foreach ($users as $user) {
            if (!forumanon_unsubscribe($user->id, $id)) {
                print_error('cannotremovesubscriber', 'forumanon', '', $user->id);
            }
        }
    }
...
$strsubscribers = get_string("subscribers", "forumanon");
$PAGE->navbar->add($strsubscribers);
$PAGE->set_title($strsubscribers);
$PAGE->set_heading($COURSE->fullname);
if (has_capability('mod/forumanon:managesubscriptions', $context)) {
    $PAGE->set_button(forumanon_update_subscriptions_button($course->id, $id));
    if ($edit != -1) {
...
echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('forum', 'forumanon').' '.$strsubscribers);
if (empty($USER->subscriptionsediting)) {
    echo $forumoutput->subscriber_overview(forumanon_subscribed_users($course, $forum, $currentgroup, $context), $forum, $course);
} else if (forumanon_is_forcesubscribed($forum)) {
