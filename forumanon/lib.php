 ...
 */
function forumanon_add_instance($forum, $mform) {
    global $CFG, $DB;
    ...
    $forum->id = $DB->insert_record('forumanon', $forum);
    ...
    if ($forum->type == 'single') {  // Create related discussion.
        ...
        $discussion->id = forumanon_add_discussion($discussion, null, $message);

        if ($mform and $draftid = file_get_submitted_draft_itemid('introeditor')) {
            // ugly hack - we need to copy the files somehow
            $discussion = $DB->get_record('forumanon_discussions', array('id'=>$discussion->id), '*', MUST_EXIST);
            $post = $DB->get_record('forumanon_posts', array('id'=>$discussion->firstpost), '*', MUST_EXIST);

            $post->message = file_save_draft_area_files($draftid, $modcontext->id, 'mod_forumanon', 'post', $post->id, array('subdirs'=>true), $post->message);
            $DB->set_field('forumanon_posts', 'message', $post->message, array('id'=>$post->id));
        }
    }
    ...
    /// course context gives the same list of users.
        $users = forumanon_get_potential_subscribers($modcontext, 0, 'u.id, u.email', '');
        foreach ($users as $user) {
            forumanon_subscribe($user->id, $forum->id);
        }
    }

    forumanon_grade_item_update($forum);
 ...
 * @return bool success
 */
function forumanon_update_instance($forum, $mform) {
    ...
    $oldforum = $DB->get_record('forumanon', array('id'=>$forum->id));
    ...
    if (($oldforum->assessed<>$forum->assessed) or ($oldforum->scale<>$forum->scale)) {
        forumanon_update_grades($forum); // recalculate grades for the forum
    }
    ...
    if ($forum->type == 'single') {  // Update related discussion and post.
        if (! $discussion = $DB->get_record('forumanon_discussions', array('forum'=>$forum->id))) {
            if ($discussions = $DB->get_records('forumanon_discussions', array('forum'=>$forum->id), 'timemodified ASC')) {
                ...
                forumanon_add_discussion($discussion, null, $message);

                if (! $discussion = $DB->get_record('forumanon_discussions', array('forum'=>$forum->id))) {
                    print_error('cannotadd', 'forumanon');
                }
            }
        }
        if (! $post = $DB->get_record('forumanon_posts', array('id'=>$discussion->firstpost))) {
        ...
        $DB->update_record('forumanon_posts', $post);
        $discussion->name = $forum->name;
        $DB->update_record('forumanon_discussions', $discussion);
    }
 ...
 * @return bool success
 */
function forumanon_delete_instance($id) {
    global $DB;

    if (!$forum = $DB->get_record('forumanon', array('id'=>$id))) {
        return false;
    }
    if (!$cm = get_coursemodule_from_instance('forumanon', $forum->id)) {
        return false;
    }
    ...
    if ($discussions = $DB->get_records('forumanon_discussions', array('forum'=>$forum->id))) {
        foreach ($discussions as $discussion) {
            if (!forumanon_delete_discussion($discussion, true, $course, $cm, $forum)) {
                $result = false;
            }
        }
    }

    if (!$DB->delete_records('forumanon_subscriptions', array('forum'=>$forum->id))) {
        $result = false;
    }

    forumanon_tp_delete_read_records(-1, -1, -1, $forum->id);

    if (!$DB->delete_records('forumanon', array('id'=>$forum->id))) {
        $result = false;
    }

    forumanon_grade_item_delete($forum);
 ...
 */
function forumanon_supports($feature) {
    switch($feature) {
 ...
 */
function forumanon_get_completion_state($course,$cm,$userid,$type) {
    global $CFG,$DB;
...
FROM
    {forumanon_posts} fp
    INNER JOIN {forumanon_discussions} fd ON fp.discussion=fd.id
WHERE
    fp.userid=:userid AND fd.forum=:forumid";

    if ($forum->completiondiscussions) {
        $value = $forum->completiondiscussions <=
                 $DB->count_records('forumanon_discussions',array('forum'=>$forum->id,'userid'=>$userid));
...
function forumanon_cron() {
    ...
    if ($posts = forumanon_get_unmailed_posts($starttime, $endtime, $timenow)) {
        // Mark them all now as being mailed.  It's unlikely but possible there
        // might be an error later so that a post is NOT actually mailed out,
        // but since mail isn't crucial, we can accept this risk.  Doing it now
        // prevents the risk of duplicated mails, which is a worse problem.

        if (!forumanon_mark_old_posts_as_mailed($endtime)) {
        ...
        foreach ($posts as $pid => $post) {
            ...
            if (!isset($forums[$forumid])) {
                if ($forum = $DB->get_record('forumanon', array('id' => $forumid))) {
            ...
            if (!isset($coursemodules[$forumid])) {
                if ($cm = get_coursemodule_from_instance('forumanon', $forumid, $courseid)) {
            ...
            if (!isset($subscribedusers[$forumid])) {
                ...
                if ($subusers = forumanon_subscribed_users($courses[$courseid], $forums[$forumid], 0, $modcontext, "u.*")) {
    ...
    if ($users && $posts) {
        ...
        foreach ($users as $userto) {
            ...
            foreach ($posts as $pid => $post) {
                ...
                if ($forum->type == 'qanda' && !forumanon_get_user_posted_time($discussion->id, $userto->id)) {
                ...
                if (!isset($userto->canpost[$discussion->id])) {
                    $modcontext = get_context_instance(CONTEXT_MODULE, $cm->id);
                    $userto->canpost[$discussion->id] = forumanon_user_can_post($forum, $discussion, $userto, $cm, $course, $modcontext);
                }
                ...
                if (!forumanon_user_can_see_post($forum, $discussion, $post, NULL, $cm)) {
                ...
                if ($userto->maildigest > 0) {
                    ...
                    $DB->insert_record('forumanon_queue', $queue);
                    continue;
                }
                ...
                $userfrom->customheaders = array (  // Headers to make emails easier to track
                           ...
                           'List-Help: '.$CFG->wwwroot.'/mod/forumanon/view.php?f='.$forum->id,
                ...
                $posttext = forumanon_make_mail_text($course, $cm, $forum, $discussion, $post, $userfrom, $userto);
                $posthtml = forumanon_make_mail_html($course, $cm, $forum, $discussion, $post, $userfrom, $userto);
                ...
                $eventdata->component        = 'mod_forumanon';
                ...
                $eventdata->smallmessage = get_string_manager()->get_string('smallmessage', 'forumanon', $smallmessagestrings, $userto->lang);

                $eventdata->contexturl = "{$CFG->wwwroot}/mod/forumanon/discuss.php?d={$discussion->id}#p{$post->id}";
                $eventdata->contexturlname = $discussion->name;

                $mailresult = message_send($eventdata);
                if (!$mailresult){
                    mtrace("Error: mod/forumanon/lib.php forumanon_cron(): Could not send out mail for id $post->id to user $userto->id".
                         " ($userto->email) .. not trying again.");
                    add_to_log($course->id, 'forumanon', 'mail error', "discuss.php?d=$discussion->id#p$post->id",
                               substr(format_string($post->subject,true),0,30), $cm->id, $userto->id);
                    $errorcount[$post->id]++;
                } else {
                    $mailcount[$post->id]++;

                // Mark post as read if forumanon_usermarksread is set off
                    if (!$CFG->forumanon_usermarksread) {
            ...
            forumanon_tp_mark_posts_read($userto, $userto->markposts);
        }
    }
    ...
    if ($posts) {
        foreach ($posts as $post) {
            ...
            if ($errorcount[$post->id]) {
                $DB->set_field("forumanon_posts", "mailed", "2", array("id" => "$post->id"));
            }
        }
    }
    ...
    $DB->delete_records_select('forumanon_queue', "timemodified < ?", array($weekago));
    ...
    if ($CFG->digestmailtimelast < $digesttime and $timenow > $digesttime) {
        ...
        $digestposts_rs = $DB->get_recordset_select('forumanon_queue', "timemodified < ?", array($digesttime));
        ...
        if ($digestposts_rs->valid()) {
            ...
            foreach ($digestposts_rs as $digestpost) {
                ...
                if (!isset($posts[$digestpost->postid])) {
                    if ($post = $DB->get_record('forumanon_posts', array('id' => $digestpost->postid))) {
                ...
                if (!isset($discussions[$discussionid])) {
                    if ($discussion = $DB->get_record('forumanon_discussions', array('id' => $discussionid))) {
                        $discussions[$discussionid] = $discussion;
                    } else {
                        continue;
                    }
            ...
            foreach ($userdiscussions as $userid => $thesediscussions) {
                ...
                mtrace(get_string('processingdigest', 'forumanon', $userid), '... ');

                // First of all delete all the queue entries for this user
                $DB->delete_records_select('forumanon_queue', "userid = ? AND timemodified < ?", array($userid, $digesttime));
                $userto = $users[$userid];
                ...
                $postsubject = get_string('digestmailsubject', 'forumanon', format_string($site->shortname, true));

                $headerdata = new stdClass();
                $headerdata->sitename = format_string($site->fullname, true);
                $headerdata->userprefs = $CFG->wwwroot.'/user/edit.php?id='.$userid.'&amp;course='.$site->id;

                $posttext = get_string('digestmailheader', 'forumanon', $headerdata)."\n\n";
                $headerdata->userprefs = '<a target="_blank" href="'.$headerdata->userprefs.'">'.get_string('digestmailprefs', 'forumanon').'</a>';
...
/*                foreach ($CFG->stylesheets as $stylesheet) {
                ...
                $posthtml .= "</head>\n<body id=\"email\">\n";
                $posthtml .= '<p>'.get_string('digestmailheader', 'forumanon', $headerdata).'</p><br /><hr size="1" noshade="noshade" />';
                ...
                foreach ($thesediscussions as $discussionid) {
                    ...
                    $canunsubscribe = ! forumanon_is_forcesubscribed($forum);
                    ...
                    "<a target=\"_blank\" href=\"$CFG->wwwroot/mod/forumanon/index.php?id=$course->id\">$strforums</a> -> ".
                    "<a target=\"_blank\" href=\"$CFG->wwwroot/mod/forumanon/view.php?f=$forum->id\">".format_string($forum->name,true)."</a>";
                    if ($discussion->name == $forum->name) {
                        $posthtml .= "</font></p>";
                    } else {
                        $posthtml .= " -> <a target=\"_blank\" href=\"$CFG->wwwroot/mod/forumanon/discuss.php?d=$discussion->id\">".format_string($discussion->name,true)."</a></font></p>";
                    }
                    $posthtml .= '<p>';
                    ...
                    foreach ($postsarray as $postid) {
                        ...
                        if ($userto->maildigest == 2) {
                            ...
                            $posttext .= "\n".format_string($post->subject,true).' '.get_string("bynameondate", "forumanon", $by);
                            $posttext .= "\n---------------------------------------------------------------------";

                            $by->name = "<a target=\"_blank\" href=\"$CFG->wwwroot/user/view.php?id=$userfrom->id&amp;course=$course->id\">$by->name</a>";
                            $posthtml .= '<div><a target="_blank" href="'.$CFG->wwwroot.'/mod/forumanon/discuss.php?d='.$discussion->id.'#p'.$post->id.'">'.format_string($post->subject,true).'</a> '.get_string("bynameondate", "forumanon", $by).'</div>';

                        } else {
                            // The full treatment
                            $posttext .= forumanon_make_mail_text($course, $cm, $forum, $discussion, $post, $userfrom, $userto, true);
                            $posthtml .= forumanon_make_mail_post($course, $cm, $forum, $discussion, $post, $userfrom, $userto, false, $canreply, true, false);
                    ...
                    if ($canunsubscribe) {
                        $posthtml .= "\n<div class='mdl-right'><font size=\"1\"><a href=\"$CFG->wwwroot/mod/forumanon/subscribe.php?id=$forum->id\">".get_string("unsubscribe", "forumanon")."</a></font></div>";
                    } else {
                        $posthtml .= "\n<div class='mdl-right'><font size=\"1\">".get_string("everyoneissubscribed", "forumanon")."</font></div>";
                    }
                ...
                $mailresult = email_to_user($userto, $site->shortname, $postsubject, $posttext, $posthtml, $attachment, $attachname, $usetrueaddress, $CFG->forumanon_replytouser);

                if (!$mailresult) {
                    mtrace("ERROR!");
                    echo "Error: mod/forumanon/cron.php: Could not send out digest mail to user $userto->id ($userto->email)... not trying again.\n";
                    add_to_log($course->id, 'forumanon', 'mail digest error', '', '', $cm->id, $userto->id);
                } else {
                    mtrace("success.");
                    $usermailcount++;
    ...
    if (!empty($usermailcount)) {
        mtrace(get_string('digestsentusers', 'forumanon', $usermailcount));
    }

    if (!empty($CFG->forum_lastreadclean)) {
        $timenow = time();
        if ($CFG->forum_lastreadclean + (24*3600) < $timenow) {
            set_config('forumanon_lastreadclean', $timenow);
            mtrace('Removing old forum read tracking info...');
            forumanon_tp_clean_read_records();
        }
    } else {
        set_config('forumanon_lastreadclean', time());
    }
 ...
 */
function forumanon_make_mail_text($course, $cm, $forum, $discussion, $post, $userfrom, $userto, $bare = false) {
    global $CFG, $USER;
    ...
    if (!isset($userto->canpost[$discussion->id])) {
        $canreply = forumanon_user_can_post($forum, $discussion, $userto, $cm, $course, $modcontext);
    } else {
        $canreply = $userto->canpost[$discussion->id];
    }
    ...
    $strbynameondate = get_string('bynameondate', 'forumanon', $by);
    ...
    $post->message = file_rewrite_pluginfile_urls($post->message, 'pluginfile.php', $modcontext->id, 'mod_forumanon', 'post', $post->id);

    $posttext .= "\n---------------------------------------------------------------------\n";
    $posttext .= format_string($post->subject,true);
    if ($bare) {
        $posttext .= " ($CFG->wwwroot/mod/forumanon/discuss.php?d=$discussion->id#p$post->id)";
    }
    $posttext .= "\n".$strbynameondate."\n";
    $posttext .= "---------------------------------------------------------------------\n";
    $posttext .= format_text_email($post->message, $post->messageformat);
    $posttext .= "\n\n";
    $posttext .= forumanon_print_attachments($post, $cm, "text");

    if (!$bare && $canreply) {
        $posttext .= "---------------------------------------------------------------------\n";
        $posttext .= get_string("postmailinfo", "forumanon", $course->shortname)."\n";
        $posttext .= "$CFG->wwwroot/mod/forumanon/post.php?reply=$post->id\n";
    }
    if (!$bare && $canunsubscribe) {
        $posttext .= "\n---------------------------------------------------------------------\n";
        $posttext .= get_string("unsubscribe", "forumanon");
        $posttext .= ": $CFG->wwwroot/mod/forumanon/subscribe.php?id=$forum->id\n";
    }
 ...
 */
function forumanon_make_mail_html($course, $cm, $forum, $discussion, $post, $userfrom, $userto) {
    global $CFG;
    ...
    if (!isset($userto->canpost[$discussion->id])) {
        $canreply = forumanon_user_can_post($forum, $discussion, $userto);
    } else {
        $canreply = $userto->canpost[$discussion->id];
    }
...
/*    foreach ($CFG->stylesheets as $stylesheet) {
    ...
    '<a target="_blank" href="'.$CFG->wwwroot.'/mod/forumanon/index.php?id='.$course->id.'">'.$strforums.'</a> &raquo; '.
    '<a target="_blank" href="'.$CFG->wwwroot.'/mod/forumanon/view.php?f='.$forum->id.'">'.format_string($forum->name,true).'</a>';
    if ($discussion->name == $forum->name) {
        $posthtml .= '</div>';
    } else {
        $posthtml .= ' &raquo; <a target="_blank" href="'.$CFG->wwwroot.'/mod/forumanon/discuss.php?d='.$discussion->id.'">'.
    ...
    if ($canunsubscribe) {
        $posthtml .= '<hr /><div class="mdl-align unsubscribelink">
                      <a href="'.$CFG->wwwroot.'/mod/forumanon/subscribe.php?id='.$forum->id.'">'.get_string('unsubscribe', 'forumanon').'</a>&nbsp;
                      <a href="'.$CFG->wwwroot.'/mod/forumanon/unsubscribeall.php">'.get_string('unsubscribeall', 'forumanon').'</a></div>';
    }
 ...
 */
function forumanon_user_outline($course, $user, $mod, $forum) {
    global $CFG;
    require_once("$CFG->libdir/gradelib.php");
    $grades = grade_get_grades($course->id, 'mod', 'forumanon', $forum->id, $user->id);
    ...
    $count = forumanon_count_user_posts($forum->id, $user->id);

    if ($count && $count->postcount > 0) {
        $result = new stdClass();
        $result->info = get_string("numposts", "forumanon", $count->postcount);
 ...
 * @param object $forum
 */
function forumanon_user_complete($course, $user, $mod, $forum) {
    ...
    if ($posts = forumanon_get_user_posts($forum->id, $user->id)) {

        if (!$cm = get_coursemodule_from_instance('forumanon', $forum->id, $course->id)) {
            print_error('invalidcoursemodule');
        }
        $discussions = forumanon_get_user_involved_discussions($forum->id, $user->id);
        ...
        echo "<p>".get_string("noposts", "forumanon")."</p>";
    }
}
 ...
 */
function forumanon_print_overview($courses,&$htmlarray) {
    ...
    if (!$forums = get_all_instances_in_courses('forumanon',$courses)) {
        return;
    }
    ...
    $sql .= ") AND l.module = 'forumanon' AND action = 'add post' "
    ...
    if (count($trackingforums) > 0) {
        $cutoffdate = isset($CFG->forumanon_oldpostdays) ? (time() - ($CFG->forumanon_oldpostdays*24*60*60)) : 0;
        $sql = 'SELECT d.forum,d.course,COUNT(p.id) AS count '.
            ' FROM {forumanon_posts} p '.
            ' JOIN {forumanon_discussions} d ON p.discussion = d.id '.
            ' LEFT JOIN {forumanon_read} r ON r.postid = p.id AND r.userid = ? WHERE (';
    ...
    $strnumunread = get_string('overviewnumunread','forumanon');
    $strnumpostssince = get_string('overviewnumpostssince','forumanon');
    ...
    foreach ($forums as $forum) {
        ...
        if ($count > 0 || $thisunread > 0) {
            $str .= '<div class="overview forum"><div class="name">'.$strforum.': <a title="'.$strforum.'" href="'.$CFG->wwwroot.'/mod/forumanon/view.php?f='.$forum->id.'">'.
                $forum->name.'</a></div>';
        ...
        if (!empty($str)) {
            ...
            if (!array_key_exists('forumanon',$htmlarray[$forum->course])) {
                $htmlarray[$forum->course]['forumanon'] = ''; // initialize, avoid warnings
            }
            $htmlarray[$forum->course]['forumanon'] .= $str;
        }
    }
}
 ...
 * @return bool success
 */
function forumanon_print_recent_activity($course, $viewfullnames, $timestart) {
    ...
    if (!$posts = $DB->get_records_sql("SELECT p.*, f.type AS forumtype, d.forum, d.groupid,
                                         ...
                                         FROM {forumanon_posts} p
                                              JOIN {forumanon_discussions} d ON d.id = p.discussion
                                              JOIN {forumanon} f             ON f.id = d.forum
    ...
    $printposts = array();
    foreach ($posts as $post) {
        if (!isset($modinfo->instances['forumanon'][$post->forum])) {
            // not visible
            continue;
        }
        $cm = $modinfo->instances['forumanon'][$post->forum];
        if (!$cm->uservisible) {
            continue;
        }
        $context = get_context_instance(CONTEXT_MODULE, $cm->id);
        ...
        if (!empty($CFG->forumanon_enabletimedposts) and $USER->id != $post->duserid
          and (($post->timestart > 0 and $post->timestart > time()) or ($post->timeend > 0 and $post->timeend < time()))) {
            if (!has_capability('mod/forumanon:viewhiddentimedposts', $context)) {
                continue;
            }
        }
    ...
    echo $OUTPUT->heading(get_string('newforumposts', 'forumanon').':', 3);
    echo "\n<ul class='unlist'>\n";
    ...
    foreach ($printposts as $post) {
        ...
        if (empty($post->parent)) {
            echo '"<a href="'.$CFG->wwwroot.'/mod/forumanon/discuss.php?d='.$post->discussion.'">';
        } else {
            echo '"<a href="'.$CFG->wwwroot.'/mod/forumanon/discuss.php?d='.$post->discussion.'&amp;parent='.$post->parent.'#p'.$post->id.'">';
        }
 ...
 */
function forumanon_get_user_grades($forum, $userid = 0) {
    global $CFG;

    require_once($CFG->dirroot.'/rating/lib.php');

    $ratingoptions = new stdClass;
    $ratingoptions->component = 'mod_forumanon';
    $ratingoptions->ratingarea = 'post';

    //need these to work backwards to get a context id. Is there a better way to get contextid from a module instance?
    $ratingoptions->modulename = 'forumanon';
    $ratingoptions->moduleid   = $forum->id;
    $ratingoptions->userid = $userid;
    $ratingoptions->aggregationmethod = $forum->assessed;
    $ratingoptions->scaleid = $forum->scale;
    $ratingoptions->itemtable = 'forumanon_posts';
 ...
 * @return void
 */
function forumanon_update_grades($forum, $userid=0, $nullifnone=true) {
    global $CFG, $DB;
    require_once($CFG->libdir.'/gradelib.php');
    ...
    } else if ($grades = forumanon_get_user_grades($forum, $userid)) {
        forumanon_grade_item_update($forum, $grades);

    } else if ($userid and $nullifnone) {
        $grade = new stdClass();
        $grade->userid   = $userid;
        $grade->rawgrade = NULL;
        forumanon_grade_item_update($forum, $grade);
 ...
 * @global object
 */
function forumanon_upgrade_grades() {
    global $DB;

    $sql = "SELECT COUNT('x')
              FROM {forumanon} f, {course_modules} cm, {modules} m
             WHERE m.name='forumanon' AND m.id=cm.module AND cm.instance=f.id";
    ...
    if ($rs->valid()) {
        ...
        foreach ($rs as $forum) {
            ...
            forumanon_update_grades($forum, 0, false);
 ...
 * @return int 0 if ok
 */
function forumanon_grade_item_update($forum, $grades=NULL) {
    global $CFG;
    ...
    return grade_update('mod/forumanon', $forum->course, 'mod', 'forumanon', $forum->id, 0, $grades, $params);
}
 ...
 */
function forumanon_grade_item_delete($forum) {
    global $CFG;
    require_once($CFG->libdir.'/gradelib.php');

    return grade_update('mod/forumanon', $forum->course, 'mod', 'forumanon', $forum->id, 0, NULL, array('deleted'=>1));
}
 ...
 */
function forumanon_get_participants($forumid) {
    ...
    $sql = "SELECT DISTINCT r.userid, r.userid AS id
              FROM {forumanon_discussions} d
              JOIN {forumanon_posts} p ON p.discussion = d.id
              JOIN {rating} r on r.itemid = p.id
             WHERE d.forum = :forumid AND
                   r.component = 'mod_forumanon' AND
                   r.ratingarea = 'post'";
 ...
 * @return bool
 */
function forumanon_scale_used ($forumid,$scaleid) {
    global $DB;
    $return = false;

    $rec = $DB->get_record("forumanon",array("id" => "$forumid","scale" => "-$scaleid"));
 ...
 */
function forumanon_scale_used_anywhere($scaleid) {
    global $DB;
    if ($scaleid and $DB->record_exists('forumanon', array('scale' => -$scaleid))) {
        return true;
    } else {
        return false;
    }
...
/**
 * Gets a post with all info ready for forumanon_print_post
 ...
 */
function forumanon_get_post_full($postid) {
    global $CFG, $DB;
...
/**
 * Gets posts with all info ready for forumanon_print_post
 ...
 * @return mixed array of posts or false
 */
function forumanon_get_discussion_posts($discussion, $sort, $forumid) {
    global $CFG, $DB;
 ...
 */
function forumanon_get_all_discussion_posts($discussionid, $sort, $tracking=false) {
    global $CFG, $DB, $USER;
    ...
    if ($tracking) {
        $now = time();
        $cutoffdate = $now - ($CFG->forumanon_oldpostdays * 24 * 3600);
        $tr_sel  = ", fr.id AS postread";
        $tr_join = "LEFT JOIN {forumanon_read} fr ON (fr.postid = p.id AND fr.userid = ?)";
        $params[] = $USER->id;
    }
 ...
 * @return array
 */
function forumanon_get_child_posts($parent, $forumid) {
    global $CFG, $DB;
 ...
 */
function forumanon_get_readable_forums($userid, $courseid=0) {

    global $CFG, $DB, $USER;
    require_once($CFG->dirroot.'/course/lib.php');

    if (!$forummod = $DB->get_record('modules', array('name' => 'forumanon'))) {
        print_error('notinstalled', 'forumanon');
    }
    ...
    foreach ($courses as $course) {
        ...
        if (empty($modinfo->instances['forumanon'])) {
            // hmm, no forums?
            continue;
        }

        $courseforums = $DB->get_records('forumanon', array('course' => $course->id));
        ...
        /// hidden timed discussions
            ...
            if (!empty($CFG->forumanon_enabletimedposts)) {
                if (!has_capability('mod/forumanon:viewhiddentimedposts', $context)) {
        ...
        /// qanda access
            if ($forum->type == 'qanda'
                    && !has_capability('mod/forumanon:viewqandawithoutposting', $context)) {
                ...
                if ($discussionspostedin = forumanon_discussions_user_has_posted_in($forum->id, $USER->id)) {
 ...
 */
function forumanon_search_posts($searchterms, $courseid=0, $limitfrom=0, $limitnum=50,
                            &$totalcount, $extrasql='') {
    global $CFG, $DB, $USER;
    require_once($CFG->libdir.'/searchlib.php');

    $forums = forumanon_get_readable_forums($USER->id, $courseid);
 ...
 */
function forumanon_get_ratings($context, $postid, $sort = "u.firstname ASC") {
    $options = new stdClass;
    $options->context = $context;
    $options->component = 'mod_forumanon';
 ...
 * @return array
 */
function forumanon_get_unmailed_posts($starttime, $endtime, $now=null) {
    global $CFG, $DB;
 ...
 * @return bool
 */
function forumanon_mark_old_posts_as_mailed($endtime, $now=null) {
    global $CFG, $DB;
    if (empty($now)) {
        $now = time();
    }
    ...
    if (empty($CFG->forumanon_enabletimedposts)) {
        return $DB->execute("UPDATE {forumanon_posts}
                               SET mailed = '1'
...
/**
 * Get all the posts for a user in a forum suitable for forumanon_print_post
 *
 * @global object
 * @global object
 * @uses CONTEXT_MODULE
 * @return array
 */
function forumanon_get_user_posts($forumid, $userid) {
    global $CFG, $DB;
    ...
    if (!empty($CFG->forumanon_enabletimedposts)) {
        $cm = get_coursemodule_from_instance('forumanon', $forumid);
        if (!has_capability('mod/forumanon:viewhiddentimedposts' , get_context_instance(CONTEXT_MODULE, $cm->id))) {
            $now = time();
            $timedsql = "AND (d.timestart < ? AND (d.timeend = 0 OR d.timeend > ?))";
            $params[] = $now;
            $params[] = $now;
    ...
    return $DB->get_records_sql("SELECT p.*, d.forum, u.firstname, u.lastname, u.email, u.picture, u.imagealt
                              FROM {forumanon} f
                                   JOIN {forumanon_discussions} d ON d.forum = f.id
                                   JOIN {forumanon_posts} p       ON p.discussion = d.id
                                   JOIN {user} u              ON u.id = p.userid
                             WHERE f.id = ?
                                   AND p.userid = ?
 ...
 */
function forumanon_get_user_involved_discussions($forumid, $userid) {
    global $CFG, $DB;
 ...
 */
function forumanon_count_user_posts($forumid, $userid) {
    global $CFG, $DB;
 ...
 * @return array|null
 */
function forumanon_get_post_from_log($log) {
    global $CFG, $DB;
 ...
 * @return array
 */
function forumanon_get_firstpost_from_discussion($discussionid) {
    global $CFG, $DB;
 ...
 * @return array
 */
function forumanon_count_discussion_replies($forumid, $forumsort="", $limit=-1, $page=-1, $perpage=0) {
    global $CFG, $DB;
 ...
 * @return mixed
 */
function forumanon_count_discussions($forum, $cm, $course) {
    global $CFG, $DB, $USER;
 ...
 * @return mixed
 */
function forumanon_count_unrated_posts($discussionid, $userid) {
    global $CFG, $DB;

    $sql = "SELECT COUNT(*) as num
              FROM {forumanon_posts}
             WHERE parent > 0
 ...
 * @return array
 */
function forumanon_get_discussions($cm, $forumsort="d.timemodified DESC", $fullpost=true, $unused=-1, $limit=-1, $userlastmodified=false, $page=-1, $perpage=0) {
    global $CFG, $DB, $USER;
    ...
    if (!has_capability('mod/forumanon:viewdiscussion', $modcontext)) { /// User must have perms to view discussions
        return array();
    }

    if (!empty($CFG->forumanon_enabletimedposts)) { /// Users must fulfill timed posts

        if (!has_capability('mod/forumanon:viewhiddentimedposts', $modcontext)) {
 ...
 * @return array
 */
function forumanon_get_discussions_unread($cm) {
    global $CFG, $DB, $USER;

    $now = round(time(), -2);
    $cutoffdate = $now - ($CFG->forumanon_oldpostdays*24*60*60);
    ...
    $sql = "SELECT d.id, COUNT(p.id) AS unread
              FROM {forumanon_discussions} d
                   JOIN {forumanon_posts} p     ON p.discussion = d.id
                   LEFT JOIN {forumanon_read} r ON (r.postid = p.id AND r.userid = $USER->id)
 ...
 * @return array
 */
function forumanon_get_discussions_count($cm) {
    global $CFG, $DB, $USER;
 ...
 * @return array
 */
function forumanon_get_user_discussions($courseid, $userid, $groupid=0) {
    global $CFG, $DB;
 ...
 */
function forumanon_get_potential_subscribers($forumcontext, $groupid, $fields, $sort) {
    global $DB;

    // only active enrolled users or everybody on the frontpage with this capability
    list($esql, $params) = get_enrolled_sql($forumcontext, 'mod/forumanon:initialsubscriptions', $groupid, true);
 ...
 * @return array list of users.
 */
function forumanon_subscribed_users($course, $forum, $groupid=0, $context = null, $fields = null) {
    global $CFG, $DB;
    ...
    if (empty($context)) {
        $cm = get_coursemodule_from_instance('forumanon', $forum->id, $course->id);
        $context = get_context_instance(CONTEXT_MODULE, $cm->id);
    }

    if (forumanon_is_forcesubscribed($forum)) {
        $results = forumanon_get_potential_subscribers($context, $groupid, $fields, "u.email ASC");
        ...
        $results = $DB->get_records_sql("SELECT $fields
                                           FROM {user} u
                                           JOIN ($esql) je ON je.id = u.id
                                           JOIN {forumanon_subscriptions} s ON s.userid = u.id
 ...
 * @param string $type
 */
function forumanon_get_course_forum($courseid, $type) {
// How to set up special 1-per-course forums
    global $CFG, $DB, $OUTPUT;

    if ($forums = $DB->get_records_select("forumanon", "course = ? AND type = ?", array($courseid, $type), "id ASC")) {
    ...
    switch ($forum->type) {
        case "news":
            ...
            break;
        case "social":
            $forum->name  = get_string("namesocial", "forumanon");
            $forum->intro = get_string("introsocial", "forumanon");
            $forum->assessed = 0;
            $forum->forcesubscribe = 0;
            break;
        case "blog":
            $forum->name = get_string('blogforum', 'forumanon');
            $forum->intro = get_string('introblog', 'forumanon');
            $forum->assessed = 0;
            $forum->forcesubscribe = 0;
            break;
        default:
    ...
    if (! $module = $DB->get_record("modules", array("name" => "forumanon"))) {
    ...
    return $DB->get_record("forumanon", array("id" => "$forum->id"));
}
 ...
 * @return string
 */
function forumanon_make_mail_post($course, $cm, $forum, $discussion, $post, $userfrom, $userto,
    ...
    $by->date = userdate($post->modified, '', $userto->timezone);
    $output .= '<div class="author">'.get_string('bynameondate', 'forumanon', $by).'</div>';
    ...
    $attachments = forumanon_print_attachments($post, $cm, 'html');
    if ($attachments !== '') {
...
// Commands
    ...
    if ($post->parent) {
        $commands[] = '<a target="_blank" href="'.$CFG->wwwroot.'/mod/forumanon/discuss.php?d='.
                      $post->discussion.'&amp;parent='.$post->parent.'">'.get_string('parent', 'forumanon').'</a>';
    }

    if ($reply) {
        $commands[] = '<a target="_blank" href="'.$CFG->wwwroot.'/mod/forumanon/post.php?reply='.$post->id.'">'.
                      get_string('reply', 'forumanon').'</a>';
    }
...
// Context link to post if required
    if ($link) {
        ...
        $output .= '<a target="_blank" href="'.$CFG->wwwroot.'/mod/forumanon/discuss.php?d='.$post->discussion.'#p'.$post->id.'">'.
                     get_string('postincontext', 'forumanon').'</a>';
        $output .= '</div>';
    }
 ...
 * @param boolean $dummyifcantsee When forumanon_user_can_see_post says that
 ...
 * @return void
 */
function forumanon_print_post($post, $discussion, $forum, &$cm, $course, $ownpost=false, $reply=false, $link=false,
    ...
    if (!isset($cm->cache->caps)) {
        $cm->cache->caps = array();
        $cm->cache->caps['mod/forumanon:viewdiscussion']   = has_capability('mod/forumanon:viewdiscussion', $modcontext);
        $cm->cache->caps['moodle/site:viewfullnames']  = has_capability('moodle/site:viewfullnames', $modcontext);
        $cm->cache->caps['mod/forumanon:editanypost']      = has_capability('mod/forumanon:editanypost', $modcontext);
        $cm->cache->caps['mod/forumanon:splitdiscussions'] = has_capability('mod/forumanon:splitdiscussions', $modcontext);
        $cm->cache->caps['mod/forumanon:deleteownpost']    = has_capability('mod/forumanon:deleteownpost', $modcontext);
        $cm->cache->caps['mod/forumanon:deleteanypost']    = has_capability('mod/forumanon:deleteanypost', $modcontext);
        $cm->cache->caps['mod/forumanon:viewanyrating']    = has_capability('mod/forumanon:viewanyrating', $modcontext);
        $cm->cache->caps['mod/forumanon:exportpost']       = has_capability('mod/forumanon:exportpost', $modcontext);
        $cm->cache->caps['mod/forumanon:exportownpost']    = has_capability('mod/forumanon:exportownpost', $modcontext);
    }
    ...
    if ($istracked && is_null($postisread)) {
        $postisread = forumanon_tp_is_post_read($USER->id, $post);
    }
    ...
    if (!forumanon_user_can_see_post($forum, $discussion, $post, NULL, $cm)) {
        ...
        $output .= html_writer::tag('div', get_string('forumsubjecthidden','forumanon'), array('class'=>'subject')); // Subject
        $output .= html_writer::tag('div', get_string('forumauthorhidden','forumanon'), array('class'=>'author')); // author
        $output .= html_writer::end_tag('div');
        $output .= html_writer::end_tag('div'); // row
        $output .= html_writer::start_tag('div', array('class'=>'row'));
        $output .= html_writer::tag('div', '&nbsp;', array('class'=>'left side')); // Groups
        $output .= html_writer::tag('div', get_string('forumbodyhidden','forumanon'), array('class'=>'content')); // Content
        $output .= html_writer::end_tag('div'); // row
    ...
    if (empty($str)) {
        $str = new stdClass;
        $str->edit         = get_string('edit', 'forumanon');
        $str->delete       = get_string('delete', 'forumanon');
        $str->reply        = get_string('reply', 'forumanon');
        $str->parent       = get_string('parent', 'forumanon');
        $str->pruneheading = get_string('pruneheading', 'forumanon');
        $str->prune        = get_string('prune', 'forumanon');
        $str->displaymode     = get_user_preferences('forumanon_displaymode', $CFG->forumanon_displaymode);
        $str->markread     = get_string('markread', 'forumanon');
        $str->markunread   = get_string('markunread', 'forumanon');
    }

    $discussionlink = new moodle_url('/mod/forumanon/discuss.php', array('d'=>$post->discussion));
    ...
    list($attachments, $attachedimages) = forumanon_print_attachments($post, $cm, 'separateimages');

    // Determine if we need to shorten this post
    $shortenpost = ($link && (strlen(strip_tags($post->message)) > $CFG->forumanon_longpost));
    ...
    if ($istracked && $CFG->forumanon_usermarksread && isloggedin()) {
    ...
    if (($ownpost && $age < $CFG->maxeditingtime) || $cm->cache->caps['mod/forumanon:editanypost']) {
        $commands[] = array('url'=>new moodle_url('/mod/forumanon/post.php', array('edit'=>$post->id)), 'text'=>$str->edit);
    }

    if ($cm->cache->caps['mod/forumanon:splitdiscussions'] && $post->parent && $forum->type != 'single') {
        $commands[] = array('url'=>new moodle_url('/mod/forumanon/post.php', array('prune'=>$post->id)), 'text'=>$str->prune, 'title'=>$str->pruneheading);
    }

    if (($ownpost && $age < $CFG->maxeditingtime && $cm->cache->caps['mod/forumanon:deleteownpost']) || $cm->cache->caps['mod/forumanon:deleteanypost']) {
        $commands[] = array('url'=>new moodle_url('/mod/forumanon/post.php', array('delete'=>$post->id)), 'text'=>$str->delete);
    }

    if ($reply) {
        $commands[] = array('url'=>new moodle_url('/mod/forumanon/post.php', array('reply'=>$post->id)), 'text'=>$str->reply);
    }

    if ($cm->cache->caps['mod/forumanon:exportpost'] || ($ownpost && $cm->cache->caps['mod/forumanon:exportownpost'])) {
    ...
    $by->date = userdate($post->modified);
    $output .= html_writer::tag('div', get_string('bynameondate', 'forumanon', $by), array('class'=>'author'));
    ...
    if ($shortenpost) {
        ...
        $postcontent  = format_text(forumanon_shorten_post($post->message), $post->messageformat, $options, $course->id);
        $postcontent .= html_writer::link($discussionlink, get_string('readtherest', 'forumanon'));
    ...
    if ($link) {
        if ($post->replies == 1) {
            $replystring = get_string('repliesone', 'forumanon', $post->replies);
        } else {
            $replystring = get_string('repliesmany', 'forumanon', $post->replies);
        }

        $output .= html_writer::start_tag('div', array('class'=>'link'));
        $output .= html_writer::link($discussionlink, get_string('discussthistopic', 'forumanon'));
        $output .= '&nbsp;('.$replystring.')';
    ...
    if ($istracked && !$CFG->forumanon_usermarksread && !$postisread) {
        forumanon_tp_mark_post_read($USER->id, $post, $forum->id);
    }
 ...
 */
function forumanon_rating_permissions($contextid, $component, $ratingarea) {
    $context = get_context_instance_by_id($contextid, MUST_EXIST);
    if ($component != 'mod_forumanon' || $ratingarea != 'post') {
        // We don't know about this component/ratingarea so just return null to get the
        // default restrictive permissions.
        return null;
    }
    return array(
        'view'    => has_capability('mod/forumanon:viewrating', $context),
        'viewany' => has_capability('mod/forumanon:viewanyrating', $context),
        'viewall' => has_capability('mod/forumanon:viewallratings', $context),
        'rate'    => has_capability('mod/forumanon:rate', $context)
    );
}
 ...
 */
function forumanon_rating_validate($params) {
    global $DB, $USER;

    // Check the component is mod_forum
    if ($params['component'] != 'mod_forumanon') {
    ...
    // Fetch all the related records ... we need to do this anyway to call forumanon_user_can_see_post
    $post = $DB->get_record('forumanon_posts', array('id' => $params['itemid'], 'userid' => $params['rateduserid']), '*', MUST_EXIST);
    $discussion = $DB->get_record('forumanon_discussions', array('id' => $post->discussion), '*', MUST_EXIST);
    ...
    if (!forumanon_user_can_see_post($forum, $discussion, $post, $USER, $cm)) {
        throw new rating_exception('nopermissiontorate');
    }
 ...
 * that calls this one: forumanon_print_latest_discussions()
 *
 * @global object
 * @global object
 ...
 */
function forumanon_print_discussion_header(&$post, $forum, $group=-1, $datestring="",
    ...
    if (!isset($rowcount)) {
        $rowcount = 0;
        $strmarkalldread = get_string('markalldread', 'forumanon');
    } else {
    ...
    echo '<a href="'.$CFG->wwwroot.'/mod/forumanon/discuss.php?d='.$post->discussion.'">'.$post->subject.'</a>';
    echo "</td>\n";
    ...
    if (has_capability('mod/forumanon:viewdiscussion', $modcontext)) {   // Show the column with replies
        ...
        if ($cantrack) {
            ...
            if ($forumtracked) {
                if ($post->unread > 0) {
                    ...
                    echo '<a href="'.$CFG->wwwroot.'/mod/forumanon/discuss.php?d='.$post->discussion.'#unread">';
                    echo $post->unread;
                    echo '</a>';
                    echo '<a title="'.$strmarkalldread.'" href="'.$CFG->wwwroot.'/mod/forumanon/markposts.php?f='.
    ...
    echo '<a href="'.$CFG->wwwroot.'/mod/forumanon/discuss.php?d='.$post->discussion.$parenturl.'">'.
 ...
 * sane place between $CFG->forumanon_longpost and $CFG->forumanon_shortpost
 *
 * @global object
 * @param string $message
 * @return string
 */
function forumanon_shorten_post($message) {
   ...
   for ($i=0; $i<$length; $i++) {
       ...
       if (!$stopzone) {
           if ($count > $CFG->forumanon_shortpost) {
               $stopzone = true;
           }
       }
   }
 ...
 */
function forumanon_print_mode_form($id, $mode, $forumtype='') {
    global $OUTPUT;
    if ($forumtype == 'single') {
        $select = new single_select(new moodle_url("/mod/forumanon/view.php", array('f'=>$id)), 'mode', forumanon_get_layout_modes(), $mode, null, "mode");
        $select->class = "forummode";
    } else {
        $select = new single_select(new moodle_url("/mod/forumanon/discuss.php", array('d'=>$id)), 'mode', forumanon_get_layout_modes(), $mode, null, "mode");
    }
 ...
 * @return string
 */
function forumanon_search_form($course, $search='') {
    global $CFG, $OUTPUT;

    $output  = '<div class="forumsearch">';
    $output .= '<form action="'.$CFG->wwwroot.'/mod/forumanon/search.php" style="display:inline">';
    $output .= '<fieldset class="invisiblefieldset">';
    $output .= $OUTPUT->help_icon('search');
    $output .= '<label class="accesshide" for="search" >'.get_string('search', 'forumanon').'</label>';
    $output .= '<input id="search" name="search" type="text" size="18" value="'.s($search, true).'" alt="search" />';
    $output .= '<label class="accesshide" for="searchforums" >'.get_string('searchforums', 'forumanon').'</label>';
    $output .= '<input id="searchforums" value="'.get_string('searchforums', 'forumanon').'" type="submit" />';
 ...
 * @return string
 */
function forumanon_go_back_to($default) {
    global $SESSION;
 ...
 * @return bool success
 */
function forumanon_move_attachments($discussion, $forumfrom, $forumto) {
    global $DB;

    $fs = get_file_storage();

    $newcm = get_coursemodule_from_instance('forumanon', $forumto);
    $oldcm = get_coursemodule_from_instance('forumanon', $forumfrom);

    $newcontext = get_context_instance(CONTEXT_MODULE, $newcm->id);
    $oldcontext = get_context_instance(CONTEXT_MODULE, $oldcm->id);

    // loop through all posts, better not use attachment flag ;-)
    if ($posts = $DB->get_records('forumanon_posts', array('discussion'=>$discussion->id), '', 'id, attachment')) {
        foreach ($posts as $post) {
            $fs->move_area_files_to_new_context($oldcontext->id,
                    $newcontext->id, 'mod_forumanon', 'post', $post->id);
            $attachmentsmoved = $fs->move_area_files_to_new_context($oldcontext->id,
                    $newcontext->id, 'mod_forumanon', 'attachment', $post->id);
 ...
 */
function forumanon_print_attachments($post, $cm, $type) {
    ...
    $strattachment = get_string('attachment', 'forumanon');
    ...
    $canexport = (has_capability('mod/forumanon:exportpost', $context) || ($post->userid == $USER->id && has_capability('mod/forumanon:exportownpost', $context)));

    require_once($CFG->libdir.'/portfoliolib.php');
    if ($files = $fs->get_area_files($context->id, 'mod_forumanon', 'attachment', $post->id, "timemodified", false)) {
        $button = new portfolio_add_button();
        foreach ($files as $file) {
            $filename = $file->get_filename();
            $mimetype = $file->get_mimetype();
            $iconimage = '<img src="'.$OUTPUT->pix_url(file_mimetype_icon($mimetype)).'" class="icon" alt="'.$mimetype.'" />';
            $path = file_encode_url($CFG->wwwroot.'/pluginfile.php', '/'.$context->id.'/mod_forumanon/attachment/'.$post->id.'/'.$filename);
 ...
 * @return array
 */
function forumanon_get_file_areas($course, $cm, $context) {
    $areas = array();
    return $areas;
}
 ...
 */
function forumanon_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload) {
    global $CFG, $DB;
    ...
    if (!$post = $DB->get_record('forumanon_posts', array('id'=>$postid))) {
        return false;
    }
    ...
    $fullpath = "/$context->id/mod_forumanon/$filearea/$postid/$relativepath";
 ...
 * @return bool
 */
function forumanon_add_attachment($post, $forum, $cm, $mform=null, &$message=null) {
    global $DB;
    ...
    file_save_draft_area_files($post->attachments, $context->id, 'mod_forumanon', 'attachment', $post->id);

    $DB->set_field('forumanon_posts', 'attachment', $present, array('id'=>$post->id));
...
function forumanon_add_new_post($post, $mform, &$message) {
    ...
    $post->message = file_save_draft_area_files($post->itemid, $context->id, 'mod_forumanon', 'post', $post->id, array('subdirs'=>true), $post->message);
    $DB->set_field('forumanon_posts', 'message', $post->message, array('id'=>$post->id));
    forumanon_add_attachment($post, $forum, $cm, $mform, $message);

    // Update discussion modified date
    $DB->set_field("forumanon_discussions", "timemodified", $post->modified, array("id" => $post->discussion));
    $DB->set_field("forumanon_discussions", "usermodified", $post->userid, array("id" => $post->discussion));
 ...
 * @return bool
 */
function forumanon_update_post($post, $mform, &$message) {
    global $USER, $CFG, $DB;
...
function forumanon_add_discussion($discussion, $mform=null, &$message=null, $userid=null) {
    ...
    if (!empty($cm->id) && !empty($discussion->itemid)) {   // In "single simple discussions" this may not exist yet
        $context = get_context_instance(CONTEXT_MODULE, $cm->id);
        $text = file_save_draft_area_files($discussion->itemid, $context->id, 'mod_forumanon', 'post', $post->id, array('subdirs'=>true), $post->message);
        $DB->set_field('forumanon_posts', 'message', $text, array('id'=>$post->id));
    }
    ...
    $post->discussion = $DB->insert_record("forumanon_discussions", $discussion);

    // Finally, set the pointer on the post.
    $DB->set_field("forumanon_posts", "discussion", $post->discussion, array("id"=>$post->id));
 ...
 * @return bool
 */
function forumanon_delete_discussion($discussion, $fulldelete, $course, $cm, $forum) {
    global $DB, $CFG;
    require_once($CFG->libdir.'/completionlib.php');

    $result = true;

    if ($posts = $DB->get_records("forumanon_posts", array("discussion" => $discussion->id))) {
        foreach ($posts as $post) {
            $post->course = $discussion->course;
            $post->forum  = $discussion->forum;
            if (!forumanon_delete_post($post, 'ignore', $course, $cm, $forum, $fulldelete)) {
                $result = false;
            }
        }
    }

    forumanon_tp_delete_read_records(-1, -1, $discussion->id);

    if (!$DB->delete_records("forumanon_discussions", array("id"=>$discussion->id))) {
        $result = false;
    }
 ...
 * @return bool
 */
function forumanon_delete_post($post, $children, $course, $cm, $forum, $skipcompletion=false) {
    global $DB, $CFG;
    require_once($CFG->libdir.'/completionlib.php');

    $context = get_context_instance(CONTEXT_MODULE, $cm->id);

    if ($children != 'ignore' && ($childposts = $DB->get_records('forumanon_posts', array('parent'=>$post->id)))) {
       if ($children) {
           foreach ($childposts as $childpost) {
               forumanon_delete_post($childpost, true, $course, $cm, $forum, $skipcompletion);
           }
       } else {
           return false;
       }
    ...
    $delopt->component = 'mod_forumanon';
    ...
    //delete attachments
    $fs = get_file_storage();
    $fs->delete_area_files($context->id, 'mod_forumanon', 'attachment', $post->id);
    $fs->delete_area_files($context->id, 'mod_forumanon', 'post', $post->id);

    if ($DB->delete_records("forumanon_posts", array("id" => $post->id))) {

        forumanon_tp_delete_read_records(-1, $post->id);

    // Just in case we are deleting the last post
        forumanon_discussion_update_last_post($post->discussion);
 ...
 * @return int
 */
function forumanon_count_replies($post, $children=true) {
    global $DB;
    $count = 0;

    if ($children) {
        if ($childposts = $DB->get_records('forumanon_posts', array('parent' => $post->id))) {
           foreach ($childposts as $childpost) {
               $count ++;                   // For this child
               $count += forumanon_count_replies($childpost, true);
           }
        }
    } else {
        $count += $DB->count_records('forumanon_posts', array('parent' => $post->id));
    }
 ...
 * @return bool
 */
function forumanon_forcesubscribe($forumid, $value=1) {
    global $DB;
    return $DB->set_field("forumanon", "forcesubscribe", $value, array("id" => $forumid));
}
...
function forumanon_get_forcesubscribed($forum) {
    global $DB;
    if (isset($forum->forcesubscribe)) {    // then we use that
        return $forum->forcesubscribe;
    } else {   // Check the database
        return $DB->get_field('forumanon', 'forcesubscribe', array('id' => $forum));
    }
}
 ...
 * @return bool
 */
function forumanon_is_subscribed($userid, $forum) {
    global $DB;
    if (is_numeric($forum)) {
        $forum = $DB->get_record('forumanon', array('id' => $forum));
    }
    if (forumanon_is_forcesubscribed($forum)) {
        return true;
    }
    return $DB->record_exists("forumanon_subscriptions", array("userid" => $userid, "forum" => $forum->id));
}

function forumanon_get_subscribed_forums($course) {
    global $USER, $CFG, $DB;
    $sql = "SELECT f.id
              FROM {forumanon} f
 ...
 * @param int $forumid
 */
function forumanon_subscribe($userid, $forumid) {
    global $DB;

    if ($DB->record_exists("forumanon_subscriptions", array("userid"=>$userid, "forum"=>$forumid))) {
        return true;
    }
    ...
    return $DB->insert_record("forumanon_subscriptions", $sub);
}
 ...
 * @param int $forumid
 */
function forumanon_unsubscribe($userid, $forumid) {
    global $DB;
    return $DB->delete_records("forumanon_subscriptions", array("userid"=>$userid, "forum"=>$forumid));
}
 ...
 * @param object $forum
 */
function forumanon_post_subscription($post, $forum) {
    ...
    switch ($action) {
        case 'subscribe':
            forumanon_subscribe($USER->id, $post->forum);
            return "<p>".get_string("nowsubscribed", "forumanon", $info)."</p>";
        case 'unsubscribe':
            forumanon_unsubscribe($USER->id, $post->forum);
            return "<p>".get_string("nownotsubscribed", "forumanon", $info)."</p>";
    }
}
 ...
 * @return string
 */
function forumanon_get_subscribe_link($forum, $context, $messages = array(), $cantaccessagroup = false, $fakelink=true, $backtoindex=false, $subscribed_forums=null) {
    global $CFG, $USER, $PAGE, $OUTPUT;
    $defaultmessages = array(
        'subscribed' => get_string('unsubscribe', 'forumanon'),
        'unsubscribed' => get_string('subscribe', 'forumanon'),
        'cantaccessgroup' => get_string('no'),
        'forcesubscribed' => get_string('everyoneissubscribed', 'forumanon'),
        'cantsubscribe' => get_string('disallowsubscribe','forumanon')
    );
    ...
    if (forumanon_is_forcesubscribed($forum)) {
        ...
        if ($subscribed) {
            ...
            $linktitle = get_string('subscribestop', 'forumanon');
        } else {
            $linktext = $messages['unsubscribed'];
            $linktitle = get_string('subscribestart', 'forumanon');
        }
        ...
        if ($fakelink) {
            $PAGE->requires->js('/mod/forumanon/forum.js');
            $PAGE->requires->js_function_call('forumanon_produce_subscribe_link', array($forum->id, $backtoindexlink, $linktext, $linktitle));
            $link = "<noscript>";
        }
        $options['id'] = $forum->id;
        $options['sesskey'] = sesskey();
        $url = new moodle_url('/mod/forumanon/subscribe.php', $options);
 ...
 * @return string
 */
function forumanon_get_tracking_link($forum, $messages=array(), $fakelink=true) {
    global $CFG, $USER, $PAGE, $OUTPUT;
    ...
    if ($fakelink) {
        $PAGE->requires->js('/mod/forumanon/forum.js');
        $PAGE->requires->js_function_call('forumanon_produce_tracking_link', Array($forum->id, $linktext, $linktitle));
        // use <noscript> to print button in case javascript is not enabled
        $link .= '<noscript>';
    }
    $url = new moodle_url('/mod/forumanon/settracking.php', array('id'=>$forum->id));
    $link .= $OUTPUT->single_button($url, $linktext, 'get', array('title'=>$linktitle));
 ...
 * @return bool
 */
function forumanon_user_has_posted_discussion($forumid, $userid) {
    global $CFG, $DB;

    $sql = "SELECT 'x'
              FROM {forumanon_discussions} d, {forumanon_posts} p
 ...
 * @return array
 */
function forumanon_discussions_user_has_posted_in($forumid, $userid) {
    global $CFG, $DB;
 ...
 * @return bool
 */
function forumanon_user_has_posted($forumid, $did, $userid) {
    global $DB;
    ...
    if (empty($did)) {
        ...
        return $DB->record_exists('forumanon_posts', array('discussion'=>$did,'userid'=>$userid));
    }
}
 ...
 */
function forumanon_get_user_posted_time($did, $userid) {
    global $DB;

    $posttime = $DB->get_field('forumanon_posts', 'MIN(created)', array('userid'=>$userid, 'discussion'=>$did));
    if (empty($posttime)) {
        return false;
    }
    return $posttime;
 ...
 * @return bool
 */
function forumanon_user_can_post_discussion($forum, $currentgroup=null, $unused=-1, $cm=NULL, $context=NULL) {
// $forum is an object
    global $USER;
    ...
    if ($forum->type == 'news') {
        $capname = 'mod/forumanon:addnews';
    } else {
        $capname = 'mod/forumanon:startdiscussion';
    }
    ...
    if ($forum->type == 'eachuser') {
        if (forumanon_user_has_posted_discussion($forum->id, $USER->id)) {
            return false;
        }
    }
 ...
 * discussion. Use forumanon_user_can_post_discussion() to check whether the user
 ...
 * @return bool
 */
function forumanon_user_can_post($forum, $discussion, $user=NULL, $cm=NULL, $course=NULL, $context=NULL) {
    global $USER, $DB;
    if (empty($user)) {
        $user = $USER;
    }
    ...
    if ($forum->type == 'news') {
        $capname = 'mod/forumanon:replynews';
    } else {
        $capname = 'mod/forumanon:replypost';
    }
 ...
 * @param object $user
 */
function forumanon_user_can_view_post($post, $course, $cm, $forum, $discussion, $user=NULL){
 ...
 * @return bool
 */
function forumanon_user_can_see_discussion($forum, $discussion, $context, $user=NULL) {
    global $USER, $DB;
    ...
    if (is_numeric($discussion)) {
        ...
        if (!$discussion = $DB->get_record('forumanon_discussions',array('id'=>$discussion))) {
            return false;
        }
    }
 ...
 * @return bool
 */
function forumanon_user_can_see_post($forum, $discussion, $post, $user=NULL, $cm=NULL) {
    global $CFG, $USER, $DB;
    ...
    if (is_numeric($post)) {
        ...
        if (!$post = $DB->get_record('forumanon_posts',array('id'=>$post))) {
            return false;
        }
    }
    ...
    if (isset($cm->cache->caps['mod/forumanon:viewdiscussion'])) {
        if (!$cm->cache->caps['mod/forumanon:viewdiscussion']) {
            return false;
        }
    } else {
        $modcontext = get_context_instance(CONTEXT_MODULE, $cm->id);
        if (!has_capability('mod/forumanon:viewdiscussion', $modcontext, $user->id)) {
            return false;
        }
    }
    ...
    if ($forum->type == 'qanda') {
        $firstpost = forumanon_get_firstpost_from_discussion($discussion->id);
        $modcontext = get_context_instance(CONTEXT_MODULE, $cm->id);
        $userfirstpost = forumanon_get_user_posted_time($discussion->id, $user->id);

        return (($userfirstpost !== false && (time() - $userfirstpost >= $CFG->maxeditingtime)) ||
                $firstpost->id == $post->id || $post->userid == $user->id || $firstpost->userid == $user->id ||
                has_capability('mod/forumanon:viewqandawithoutposting', $modcontext, $user->id, false));
    }
    return true;
}
 ...
 *
 */
function forumanon_print_latest_discussions($course, $forum, $maxdiscussions=-1, $displayformat='plain', $sort='',
...
// and the current user is a guest.
    ...
    $canstart = forumanon_user_can_post_discussion($forum, $currentgroup, $groupmode, $cm, $context);
    ...
    if ($canstart) {
        ...
        echo "<form id=\"newdiscussionform\" method=\"get\" action=\"$CFG->wwwroot/mod/forumanon/post.php\">";
        echo '<div>';
        echo "<input type=\"hidden\" name=\"forum\" value=\"$forum->id\" />";
        switch ($forum->type) {
            case 'news':
            case 'blog':
                $buttonadd = get_string('addanewtopic', 'forumanon');
                break;
            case 'qanda':
                $buttonadd = get_string('addanewquestion', 'forumanon');
                break;
            default:
                $buttonadd = get_string('addanewdiscussion', 'forumanon');
                break;
        }
    ...
    } else if ($groupmode and has_capability('mod/forumanon:startdiscussion', $context)) {
        // inform users why they can not post new discussion
        if ($currentgroup) {
            echo $OUTPUT->notification(get_string('cannotadddiscussion', 'forumanon'));
        } else {
            echo $OUTPUT->notification(get_string('cannotadddiscussionall', 'forumanon'));
        }
    }
...
// Get all the recent discussions we're allowed to see
    ...
    if (! $discussions = forumanon_get_discussions($cm, $sort, $fullpost, null, $maxdiscussions, $getuserlastmodified, $page, $perpage) ) {
        echo '<div class="forumnodiscuss">';
        if ($forum->type == 'news') {
            echo '('.get_string('nonews', 'forumanon').')';
        } else if ($forum->type == 'qanda') {
            echo '('.get_string('noquestions','forumanon').')';
        } else {
            echo '('.get_string('nodiscussions', 'forumanon').')';
        }
        echo "</div>\n";
        return;
    }
...
// If we want paging
    if ($page != -1) {
        ...
        $numdiscussions = forumanon_get_discussions_count($cm);
        ...
        if ($numdiscussions > 1000) {
            ...
            $replies = forumanon_count_discussion_replies($forum->id, $sort, $maxdiscussions, $page, $perpage);
        } else {
            $replies = forumanon_count_discussion_replies($forum->id);
        }
    ...
    if ($cantrack = forumanon_tp_can_track_forums($forum)) {
    ...
    if ($forumtracked) {
        $unreads = forumanon_get_discussions_unread($cm);
    } else {
        $unreads = array();
    }
    ...
    if ($displayformat == 'header') {
        ...
        echo '<thead>';
        echo '<tr>';
        echo '<th class="header topic" scope="col">'.get_string('discussion', 'forumanon').'</th>';
        echo '<th class="header author" colspan="2" scope="col">'.get_string('startedby', 'forumanon').'</th>';
        if ($groupmode > 0) {
            echo '<th class="header group" scope="col">'.get_string('group').'</th>';
        }
        if (has_capability('mod/forumanon:viewdiscussion', $context)) {
            echo '<th class="header replies" scope="col">'.get_string('replies', 'forumanon').'</th>';
            // If the forum can be tracked, display the unread column.
            if ($cantrack) {
                echo '<th class="header replies" scope="col">'.get_string('unread', 'forumanon');
                if ($forumtracked) {
                    echo '&nbsp;<a title="'.get_string('markallread', 'forumanon').
                         '" href="'.$CFG->wwwroot.'/mod/forumanon/markposts.php?f='.
                         $forum->id.'&amp;mark=read&amp;returnpage=view.php">'.
                         '<img src="'.$OUTPUT->pix_url('t/clear') . '" class="iconsmall" alt="'.get_string('markallread', 'forumanon').'" /></a>';
                }
                echo '</th>';
            }
        }
        echo '<th class="header lastpost" scope="col">'.get_string('lastpost', 'forumanon').'</th>';
        echo '</tr>';
        echo '</thead>';
        echo '<tbody>';
    }
    ...
    foreach ($discussions as $discussion) {
        ...
        switch ($displayformat) {
            case 'header':
                ...
                forumanon_print_discussion_header($discussion, $forum, $group, $strdatestring, $cantrack, $forumtracked,
            ...
            default:
                ...
                if ($discussion->replies) {
                    ...
                    $modcontext = get_context_instance(CONTEXT_MODULE, $cm->id);
                    $link = forumanon_user_can_post($forum, $discussion, $USER, $cm, $course, $modcontext);
                }

                $discussion->forum = $forum->id;

                forumanon_print_post($discussion, $discussion, $forum, $cm, $course, $ownpost, 0, $link, false);
            break;
        }
    }
    ...
    if ($olddiscussionlink) {
        if ($forum->type == 'news') {
            $strolder = get_string('oldertopics', 'forumanon');
        } else {
            $strolder = get_string('olderdiscussions', 'forumanon');
        }
        echo '<div class="forumolddiscuss">';
        echo '<a href="'.$CFG->wwwroot.'/mod/forumanon/view.php?f='.$forum->id.'&amp;showall=1">';
        echo $strolder.'</a> ...</div>';
    }
 ...
 * @param bool $canrate
 */
function forumanon_print_discussion($course, $cm, $forum, $discussion, $post, $mode, $canreply=NULL, $canrate=false) {
    global $USER, $CFG;
    ...
    $modcontext = get_context_instance(CONTEXT_MODULE, $cm->id);
    if ($canreply === NULL) {
        $reply = forumanon_user_can_post($forum, $discussion, $USER, $cm, $course, $modcontext);
    } else {
        $reply = $canreply;
    }
    ...
    $forumtracked = forumanon_tp_is_tracked($forum);
    $posts = forumanon_get_all_discussion_posts($discussion->id, $sort, $forumtracked);
    $post = $posts[$post->id];
    ...
    if ($forum->assessed != RATING_AGGREGATE_NONE) {
        ...
        if ($forum->type == 'single' or !$discussion->id) {
            $ratingoptions->returnurl = "$CFG->wwwroot/mod/forumanon/view.php?id=$cm->id";
        } else {
            $ratingoptions->returnurl = "$CFG->wwwroot/mod/forumanon/discuss.php?d=$discussion->id";
        }
    ...
    $post->forum = $forum->id;   // Add the forum id to the post object, later used by forumanon_print_post
    ...
    forumanon_print_post($post, $discussion, $forum, $cm, $course, $ownpost, $reply, false,
 ...
 * @return void
 */
function forumanon_print_posts_flat($course, &$cm, $forum, $discussion, $post, $mode, $reply, $forumtracked, $posts) {
    global $USER, $CFG;
    ...
    foreach ($posts as $post) {
        ...
        forumanon_print_post($post, $discussion, $forum, $cm, $course, $ownpost, $reply, $link,
                             '', '', $postread, true, $forumtracked);
    }
}
 ...
 * @return void
 */
function forumanon_print_posts_threaded($course, &$cm, $forum, $discussion, $parent, $depth, $reply, $forumtracked, $posts) {
    global $USER, $CFG;
    ...
    if (!empty($posts[$parent->id]->children)) {
        ...
        foreach ($posts as $post) {
            ...
            if ($depth > 0) {
                ...
                print_string("bynameondate", "forumanon", $by);
                echo "</span>";
            }

            forumanon_print_posts_threaded($course, $cm, $forum, $discussion, $post, $depth-1, $reply, $forumtracked, $posts);
            echo "</div>\n";
        }
    }
}
 ...
 * @return void
 */
function forumanon_print_posts_nested($course, &$cm, $forum, $discussion, $parent, $reply, $forumtracked, $posts) {
    global $USER, $CFG;
 ...
 * @global object
 */
function forumanon_get_recent_mod_activity(&$activities, &$index, $timestart, $courseid, $cmid, $userid=0, $groupid=0)  {
    ...
    $viewhiddentimed = has_capability('mod/forumanon:viewhiddentimedposts', $cm_context);
 ...
 * @global object
 */
function forumanon_print_recent_mod_activity($activity, $courseid, $detail, $modnames, $viewfullnames) {
    global $CFG, $OUTPUT;
    ...
    echo "<a href=\"$CFG->wwwroot/mod/forumanon/discuss.php?d={$activity->content->discussion}"
 ...
 * @return bool
 */
function forumanon_change_discussionid($postid, $discussionid) {
    global $DB;
    $DB->set_field('forumanon_posts', 'discussion', $discussionid, array('id' => $postid));
    if ($posts = $DB->get_records('forumanon_posts', array('parent' => $postid))) {
        foreach ($posts as $post) {
            forumanon_change_discussionid($post->id, $discussionid);
        }
    }
    return true;
}
 ...
 * @return string
 */
function forumanon_update_subscriptions_button($courseid, $forumid) {
    global $CFG, $USER;
    ...
    return "<form method=\"get\" action=\"$CFG->wwwroot/mod/forumanon/subscribers.php\">".
 ...
 * @return void
 */
function forumanon_user_enrolled($cp) {
    $context = get_context_instance(CONTEXT_COURSE, $cp->courseid);
    forumanon_add_user_default_subscriptions($cp->userid, $context);
}
 ...
 * @return void
 */
function forumanon_user_unenrolled($cp) {
    if ($cp->lastenrol) {
        $context = get_context_instance(CONTEXT_COURSE, $cp->courseid);
        forumanon_remove_user_subscriptions($cp->userid, $context);
        forumanon_remove_user_tracking($cp->userid, $context);
    }
}
 ...
 * @return bool
 */
function forumanon_add_user_default_subscriptions($userid, $context) {
    global $DB;
    ...
    switch ($context->contextlevel) {
        ...
        case CONTEXT_SYSTEM:   // For the whole site
             ...
             foreach ($rs as $course) {
                 ...
                 forumanon_add_user_default_subscriptions($userid, $subcontext);
             }
             $rs->close();
             break;
                     ...
                     if ($forums = get_all_instances_in_course('forumanon', $course, $userid, false)) {
                         foreach ($forums as $forum) {
                             ...
                             if ($modcontext = get_context_instance(CONTEXT_MODULE, $forum->coursemodule)) {
                                 if (has_capability('mod/forumanon:viewdiscussion', $modcontext, $userid)) {
                                     forumanon_subscribe($userid, $forum->id);
                                 }
                             }
                         }
                     }
 ...
 * @return bool
 */
function forumanon_remove_user_subscriptions($userid, $context) {
    ...
    switch ($context->contextlevel) {
        ...
        case CONTEXT_SYSTEM:   // For the whole site
            ...
            if ($courses = $DB->get_records_sql("SELECT c.id
                ...
                foreach ($courses as $course) {
                    $subcontext = get_context_instance(CONTEXT_COURSE, $course->id);
                    forumanon_remove_user_subscriptions($userid, $subcontext);
                }
            }
            break;
                 ...
                 if ($course = $DB->get_record('course', array('id' => $context->instanceid), 'id')) {
                    ...
                    if ($forums = $DB->get_records_sql("SELECT f.id, cm.id as coursemodule
                                                        ...
                                                        WHERE fs.userid = ? AND f.course = ?
                                                              ...
                                                              AND cm.module = m.id AND m.name = 'forumanon'", array($userid, $context->instanceid))) {

                         foreach ($forums as $forum) {
                             if ($modcontext = get_context_instance(CONTEXT_MODULE, $forum->coursemodule)) {
                                 if (!has_capability('mod/forumanon:viewdiscussion', $modcontext, $userid)) {
                                     forumanon_unsubscribe($userid, $forum->id);
                                 }
                             }
                         }
                     }
 ...
 * @return bool
 */
function forumanon_remove_user_tracking($userid, $context) {
    ...
    switch ($context->contextlevel) {
        ...
        case CONTEXT_SYSTEM:   // For the whole site
            ...
            foreach ($allcourses as $course) {
                $subcontext = get_context_instance(CONTEXT_COURSE, $course->id);
                forumanon_remove_user_tracking($userid, $subcontext);
            }
            break;
                 ...
                 if ($course = $DB->get_record('course', array('id' => $context->instanceid), 'id')) {
                    ...
                    if ($forums = $DB->get_records_sql("SELECT DISTINCT f.id, cm.id as coursemodule
                         ...
                         foreach ($forums as $forum) {
                             if ($modcontext = get_context_instance(CONTEXT_MODULE, $forum->coursemodule)) {
                                 if (!has_capability('mod/forumanon:viewdiscussion', $modcontext, $userid)) {
                                    forumanon_tp_delete_read_records($userid, -1, -1, $forum->id);
                                 }
                             }
                         }
                     }
                    ...
                    if ($forums = $DB->get_records_sql("SELECT f.id, cm.id as coursemodule
                         ...
                         foreach ($forums as $forum) {
                             if ($modcontext = get_context_instance(CONTEXT_MODULE, $forum->coursemodule)) {
                                 if (!has_capability('mod/forumanon:viewdiscussion', $modcontext, $userid)) {
                                    $DB->delete_records('forumanon_track_prefs', array('userid' => $userid, 'forumid' => $forum->id));
                                 }
                             }
                         }
                     }
 ...
 */
function forumanon_tp_mark_posts_read($user, $postids) {
    global $CFG, $DB;

    if (!forumanon_tp_can_track_forums(false, $user)) {
        return true;
    }
    ...
    if (empty($postids)) {
        ...
        while ($part = array_splice($postids, 0, 200)) {
            $status = forumanon_tp_mark_posts_read($user, $part) && $status;
        }
        return $status;
    }
    ...
    if ($new) {
        ...
        $sql = "INSERT INTO {forumanon_read} (userid, postid, discussionid, forumid, firstread, lastread)
 ...
 * @param int $postid
 */
function forumanon_tp_add_read_record($userid, $postid) {
    global $CFG, $DB;

    $now = time();
    $cutoffdate = $now - ($CFG->forumanon_oldpostdays * 24 * 3600);

    if (!$DB->record_exists('forumanon_read', array('userid' => $userid, 'postid' => $postid))) {
        $sql = "INSERT INTO {forumanon_read} (userid, postid, discussionid, forumid, firstread, lastread)
...
/**
 * Returns all records in the 'forumanon_read' table matching the passed keys, indexed
 * by userid.
 *
 * @global object
 * @param int $userid
 ...
 * @return array
 */
function forumanon_tp_get_read_records($userid=-1, $postid=-1, $discussionid=-1, $forumid=-1) {
    global $DB;
    $select = '';
    $params = array();
    ...
    return $DB->get_records_select('forumanon_read', $select, $params);
}
 ...
 * @param int $discussionid
 */
function forumanon_tp_get_discussion_read_records($userid, $discussionid) {
    global $DB;
    $select = 'userid = ? AND discussionid = ?';
    $fields = 'postid, firstread, lastread';
    return $DB->get_records_select('forumanon_read', $select, array($userid, $discussionid), '', $fields);
}
 ...
 * @return bool
 */
function forumanon_tp_mark_post_read($userid, $post, $forumid) {
    if (!forumanon_tp_is_post_old($post)) {
        return forumanon_tp_add_read_record($userid, $post->id);
    } else {
        return true;
    }
}
 ...
 * @return bool
 */
function forumanon_tp_mark_forum_read($user, $forumid, $groupid=false) {
    global $CFG, $DB;

    $cutoffdate = time() - ($CFG->forumanon_oldpostdays*24*60*60);
    ...
    $sql = "SELECT p.id
              FROM {forumanon_posts} p
                   LEFT JOIN {forumanon_discussions} d ON d.id = p.discussion
                   LEFT JOIN {forumanon_read} r        ON (r.postid = p.id AND r.userid = ?)
             WHERE d.forum = ?
    ...
    if ($posts = $DB->get_records_sql($sql, $params)) {
        ...
        return forumanon_tp_mark_posts_read($user, $postids);
    }
 ...
 * @return bool
 */
function forumanon_tp_mark_discussion_read($user, $discussionid) {
    global $CFG, $DB;
 ...
 * @param object $post
 */
function forumanon_tp_is_post_read($userid, $post) {
    global $DB;
    return (forumanon_tp_is_post_old($post) ||
            $DB->record_exists('forumanon_read', array('userid' => $userid, 'postid' => $post->id)));
}
 ...
 */
function forumanon_tp_is_post_old($post, $time=null) {
    global $CFG;

    if (is_null($time)) {
        $time = time();
    }
    return ($post->modified < ($time - ($CFG->forumanon_oldpostdays * 24 * 3600)));
}
 ...
 * @return bool
 */
function forumanon_tp_count_discussion_read_records($userid, $discussionid) {
    global $CFG, $DB;
    ...
    $sql = 'SELECT COUNT(DISTINCT p.id) '.
           'FROM {forumanon_discussions} d '.
           'LEFT JOIN {forumanon_read} r ON d.id = r.discussionid AND r.userid = ? '.
           'LEFT JOIN {forumanon_posts} p ON p.discussion = d.id '.
 ...
 * @return int
 */
function forumanon_tp_count_discussion_unread_posts($userid, $discussionid) {
    global $CFG, $DB;
 ...
 * @return int
 */
function forumanon_tp_count_forum_posts($forumid, $groupid=false) {
    global $CFG, $DB;
    $params = array($forumid);
    $sql = 'SELECT COUNT(*) '.
           'FROM {forumanon_posts} fp,{forumanon_discussions} fd '.
 ...
 * @return int
 */
function forumanon_tp_count_forum_read_records($userid, $forumid, $groupid=false) {
    global $CFG, $DB;
 ...
 * @return array
 */
function forumanon_tp_get_course_unread_posts($userid, $courseid) {
    global $CFG, $DB;
 ...
 * @return int
 */
function forumanon_tp_count_forum_unread_posts($cm, $course) {
    global $CFG, $USER, $DB;
    ...
    if (!isset($readcache[$course->id])) {
        ...
        if ($counts = forumanon_tp_get_course_unread_posts($USER->id, $course->id)) {
            foreach ($counts as $count) {
 ...
 * @return bool
 */
function forumanon_tp_delete_read_records($userid=-1, $postid=-1, $discussionid=-1, $forumid=-1) {
    global $DB;
    $params = array();
    ...
    if ($select == '') {
        ...
        return $DB->delete_records_select('forumanon_read', $select, $params);
    }
}
/**
 ...
 */
function forumanon_tp_get_untracked_forums($userid, $courseid) {
    global $CFG, $DB;
 ...
 * @return boolean
 */
function forumanon_tp_can_track_forums($forum=false, $user=false) {
    global $USER, $CFG, $DB;

    // if possible, avoid expensive
    // queries
    if (empty($CFG->forumanon_trackreadposts)) {
        return false;
    }
 ...
 * @return boolean
 */
function forumanon_tp_is_tracked($forum, $user=false) {
    global $USER, $CFG, $DB;
    ...
    if (!forumanon_tp_can_track_forums($forum, $user)) {
        return false;
    }
 ...
 * @param int $userid
 */
function forumanon_tp_start_tracking($forumid, $userid=false) {
    global $USER, $DB;
    ...
    return $DB->delete_records('forumanon_track_prefs', array('userid' => $userid, 'forumid' => $forumid));
}
 ...
 * @param int $userid
 */
function forumanon_tp_stop_tracking($forumid, $userid=false) {
    global $USER, $DB;
    ...
    if (!$DB->record_exists('forumanon_track_prefs', array('userid' => $userid, 'forumid' => $forumid))) {
        $track_prefs = new stdClass();
        $track_prefs->userid = $userid;
        $track_prefs->forumid = $forumid;
        $DB->insert_record('forumanon_track_prefs', $track_prefs);
    }

    return forumanon_tp_delete_read_records($userid, -1, -1, $forumid);
}
 ...
 * @return void
 */
function forumanon_tp_clean_read_records() {
    global $CFG, $DB;

    if (!isset($CFG->forumanon_oldpostdays)) {
        return;
    }
// Look for records older than the cutoffdate that are still in the forum_read table.
    ...
    $sql = "SELECT MIN(fp.modified) AS first
              FROM {forumanon_posts} fp
                   JOIN {forumanon_read} fr ON fr.postid=fp.id";
 ...
 * @return bool|int
 **/
function forumanon_discussion_update_last_post($discussionid) {
    global $CFG, $DB;

// Check the given discussion exists
    if (!$DB->record_exists('forumanon_discussions', array('id' => $discussionid))) {
        return false;
    }
...
// Lets go find the last post
    if (($lastposts = $DB->get_records_sql($sql, array($discussionid), 0, 1))) {
        ...
        $DB->update_record('forumanon_discussions', $discussionobject);
        return $lastpost->id;
    }
 ...
 * @return array
 */
function forumanon_get_view_actions() {
 ...
 * @return array
 */
function forumanon_get_post_actions() {
 ...
 * @return array
 */
function forumanon_get_separate_modules($courseid) {

    global $CFG,$DB;
    $forummodule = $DB->get_record("modules", array("name" => "forumanon"));

    $sql = 'SELECT f.id, f.id FROM {forumanon} f, {course_modules} cm WHERE
 ...
 * @return bool
 */
function forumanon_check_throttling($forum, $cm=null) {
    ...
    $modcontext = get_context_instance(CONTEXT_MODULE, $cm->id);
    if(has_capability('mod/forumanon:postwithoutthrottling', $modcontext)) {
        return true;
    }
    ...
    $numposts = $DB->count_records_sql('SELECT COUNT(p.id) FROM {forumanon_posts} p'
                                      .' JOIN {forumanon_discussions} d'
    ...
    if ($forum->blockafter <= $numposts) {
        print_error('forumblockingtoomanyposts', 'error', $CFG->wwwroot.'/mod/forumanon/view.php?f='.$forum->id, $a);
    }
    if ($forum->warnafter <= $numposts) {
        echo $OUTPUT->notification(get_string('forumblockingalmosttoomanyposts','forumanon',$a));
    }
 ...
 */
function forumanon_reset_gradebook($courseid, $type='') {
    global $CFG, $DB;
    ...
    $sql = "SELECT f.*, cm.idnumber as cmidnumber, f.course as courseid
              FROM {forumanon} f, {course_modules} cm, {modules} m
             WHERE m.name='forumanon' AND m.id=cm.module AND cm.instance=f.id AND f.course=? $wheresql";

    if ($forums = $DB->get_records_sql($sql, $params)) {
        foreach ($forums as $forum) {
            forumanon_grade_item_update($forum, 'reset');
        }
    }
}
 ...
 */
function forumanon_reset_userdata($data) {
    global $CFG, $DB;
    require_once($CFG->dirroot.'/rating/lib.php');

    $componentstr = get_string('modulenameplural', 'forumanon');
    $status = array();
    ...
    if (!empty($data->reset_forum_all)) {
        $removeposts = true;
        $typesstr    = get_string('resetforumsall', 'forumanon');
        $types       = array();
    } else if (!empty($data->reset_forum_types)){
        $removeposts = true;
        $typesql     = "";
        $types       = array();
        $forum_types_all = forumanon_get_forum_types_all();
        ...
        $typesstr = get_string('resetforums', 'forumanon').': '.implode(', ', $types);
    }
    $alldiscussionssql = "SELECT fd.id
                            FROM {forumanon_discussions} fd, {forumanon} f
    ...
    $allpostssql       = "SELECT fp.id
                            FROM {forumanon_posts} fp, {forumanon_discussions} fd, {forumanon} f
    ...
    if( $removeposts || !empty($data->reset_forum_ratings) ) {
        ...
        $ratingdeloptions->component = 'mod_forumanon';
    ...
    if ($removeposts) {
        ...
        if ($forums) {
            foreach ($forums as $forumid=>$unused) {
                if (!$cm = get_coursemodule_from_instance('forumanon', $forumid)) {
                    continue;
                }
                $context = get_context_instance(CONTEXT_MODULE, $cm->id);
                $fs->delete_area_files($context->id, 'mod_forumanon', 'attachment');
                $fs->delete_area_files($context->id, 'mod_forumanon', 'post');
        ...
        $DB->delete_records_select('forumanon_read', "forumid IN ($forumssql)", $params);

        // remove tracking prefs
        $DB->delete_records_select('forumanon_track_prefs', "forumid IN ($forumssql)", $params);

        // remove posts from queue
        $DB->delete_records_select('forumanon_queue', "discussionid IN ($discussionssql)", $params);

        // all posts - initial posts must be kept in single simple discussion forums
        $DB->delete_records_select('forumanon_posts', "discussion IN ($discussionssql) AND parent <> 0", $params); // first all children
        $DB->delete_records_select('forumanon_posts', "discussion IN ($discussionssql AND f.type <> 'single') AND parent = 0", $params); // now the initial posts for non single simple

        // finally all discussions except single simple forums
        $DB->delete_records_select('forumanon_discussions', "forum IN ($forumssql AND f.type <> 'single')", $params);

        // remove all grades from gradebook
        if (empty($data->reset_gradebook_grades)) {
            if (empty($types)) {
                forumanon_reset_gradebook($data->courseid);
            } else {
                foreach ($types as $type) {
                    forumanon_reset_gradebook($data->courseid, $type);
                }
            }
        }
    ...
    if (!empty($data->reset_forum_subscriptions)) {
        $DB->delete_records_select('forumanon_subscriptions', "forum IN ($allforumssql)", $params);
        $status[] = array('component'=>$componentstr, 'item'=>get_string('resetsubscriptions','forumanon'), 'error'=>false);
    }

    // remove all tracking prefs unconditionally - even for users still enrolled in course
    if (!empty($data->reset_forum_track_prefs)) {
        $DB->delete_records_select('forumanon_track_prefs', "forumid IN ($allforumssql)", $params);
        $status[] = array('component'=>$componentstr, 'item'=>get_string('resettrackprefs','forumanon'), 'error'=>false);
    }

    /// updating dates - shift may be negative too
    if ($data->timeshift) {
        shift_course_mod_dates('forumanon', array('assesstimestart', 'assesstimefinish'), $data->timeshift, $data->courseid);
 ...
 */
function forumanon_reset_course_form_definition(&$mform) {
    $mform->addElement('header', 'forumheader', get_string('modulenameplural', 'forumanon'));

    $mform->addElement('checkbox', 'reset_forum_all', get_string('resetforumsall','forumanon'));

    $mform->addElement('select', 'reset_forum_types', get_string('resetforums', 'forumanon'), forumanon_get_forum_types_all(), array('multiple' => 'multiple'));
    $mform->setAdvanced('reset_forum_types');
    $mform->disabledIf('reset_forum_types', 'reset_forum_all', 'checked');

    $mform->addElement('checkbox', 'reset_forum_subscriptions', get_string('resetsubscriptions','forumanon'));
    $mform->setAdvanced('reset_forum_subscriptions');

    $mform->addElement('checkbox', 'reset_forum_track_prefs', get_string('resettrackprefs','forumanon'));
 ...
 * @return array
 */
function forumanon_reset_course_form_defaults($course) {
 ...
 */
function forumanon_convert_to_roles($forum, $forummodid, $teacherroles=array(),
    ...
    if ($forum->type == 'teacher') {
        ...
        if ($DB->count_records('forumanon_discussions', array('forum' => $forum->id)) == 0) {
            // Delete empty teacher forums.
            $DB->delete_records('forumanon', array('id' => $forum->id));
        } else {
            ...
            if (!$cmid = add_course_module($mod)) {
                print_error('cannotcreateinstanceforteacher', 'forumanon');
            } else {
                $mod->coursemodule = $cmid;
                if (!$sectionid = add_mod_to_section($mod)) {
                    print_error('cannotaddteacherforumto', 'forumanon');
                } else {
            ...
            foreach ($studentroles as $studentrole) {
                assign_capability('mod/forumanon:viewdiscussion', CAP_PREVENT, $studentrole->id, $context->id);
                assign_capability('mod/forumanon:viewhiddentimedposts', CAP_PREVENT, $studentrole->id, $context->id);
                assign_capability('mod/forumanon:startdiscussion', CAP_PREVENT, $studentrole->id, $context->id);
                assign_capability('mod/forumanon:replypost', CAP_PREVENT, $studentrole->id, $context->id);
                assign_capability('mod/forumanon:viewrating', CAP_PREVENT, $studentrole->id, $context->id);
                assign_capability('mod/forumanon:viewanyrating', CAP_PREVENT, $studentrole->id, $context->id);
                assign_capability('mod/forumanon:rate', CAP_PREVENT, $studentrole->id, $context->id);
                assign_capability('mod/forumanon:createattachment', CAP_PREVENT, $studentrole->id, $context->id);
                assign_capability('mod/forumanon:deleteownpost', CAP_PREVENT, $studentrole->id, $context->id);
                assign_capability('mod/forumanon:deleteanypost', CAP_PREVENT, $studentrole->id, $context->id);
                assign_capability('mod/forumanon:splitdiscussions', CAP_PREVENT, $studentrole->id, $context->id);
                assign_capability('mod/forumanon:movediscussions', CAP_PREVENT, $studentrole->id, $context->id);
                assign_capability('mod/forumanon:editanypost', CAP_PREVENT, $studentrole->id, $context->id);
                assign_capability('mod/forumanon:viewqandawithoutposting', CAP_PREVENT, $studentrole->id, $context->id);
                assign_capability('mod/forumanon:viewsubscribers', CAP_PREVENT, $studentrole->id, $context->id);
                assign_capability('mod/forumanon:managesubscriptions', CAP_PREVENT, $studentrole->id, $context->id);
                assign_capability('mod/forumanon:postwithoutthrottling', CAP_PREVENT, $studentrole->id, $context->id);
            }
            foreach ($guestroles as $guestrole) {
                assign_capability('mod/forumanon:viewdiscussion', CAP_PREVENT, $guestrole->id, $context->id);
                assign_capability('mod/forumanon:viewhiddentimedposts', CAP_PREVENT, $guestrole->id, $context->id);
                assign_capability('mod/forumanon:startdiscussion', CAP_PREVENT, $guestrole->id, $context->id);
                assign_capability('mod/forumanon:replypost', CAP_PREVENT, $guestrole->id, $context->id);
                assign_capability('mod/forumanon:viewrating', CAP_PREVENT, $guestrole->id, $context->id);
                assign_capability('mod/forumanon:viewanyrating', CAP_PREVENT, $guestrole->id, $context->id);
                assign_capability('mod/forumanon:rate', CAP_PREVENT, $guestrole->id, $context->id);
                assign_capability('mod/forumanon:createattachment', CAP_PREVENT, $guestrole->id, $context->id);
                assign_capability('mod/forumanon:deleteownpost', CAP_PREVENT, $guestrole->id, $context->id);
                assign_capability('mod/forumanon:deleteanypost', CAP_PREVENT, $guestrole->id, $context->id);
                assign_capability('mod/forumanon:splitdiscussions', CAP_PREVENT, $guestrole->id, $context->id);
                assign_capability('mod/forumanon:movediscussions', CAP_PREVENT, $guestrole->id, $context->id);
                assign_capability('mod/forumanon:editanypost', CAP_PREVENT, $guestrole->id, $context->id);
                assign_capability('mod/forumanon:viewqandawithoutposting', CAP_PREVENT, $guestrole->id, $context->id);
                assign_capability('mod/forumanon:viewsubscribers', CAP_PREVENT, $guestrole->id, $context->id);
                assign_capability('mod/forumanon:managesubscriptions', CAP_PREVENT, $guestrole->id, $context->id);
                assign_capability('mod/forumanon:postwithoutthrottling', CAP_PREVENT, $guestrole->id, $context->id);
            }
        }
    } else {
        // Non-teacher forum.
        ...
        switch ($forum->open) {
            ...
            case 1:
                foreach ($studentroles as $studentrole) {
                    assign_capability('mod/forumanon:startdiscussion', CAP_PREVENT, $studentrole->id, $context->id);
                    assign_capability('mod/forumanon:replypost', CAP_ALLOW, $studentrole->id, $context->id);
                }
                break;
            case 2:
                foreach ($studentroles as $studentrole) {
                    assign_capability('mod/forumanon:startdiscussion', CAP_ALLOW, $studentrole->id, $context->id);
                    assign_capability('mod/forumanon:replypost', CAP_ALLOW, $studentrole->id, $context->id);
                }
                break;
        }
        ...
        switch ($forum->assessed) {
            case 1:
                foreach ($studentroles as $studentrole) {
                    assign_capability('mod/forumanon:rate', CAP_ALLOW, $studentrole->id, $context->id);
                }
                foreach ($teacherroles as $teacherrole) {
                    assign_capability('mod/forumanon:rate', CAP_ALLOW, $teacherrole->id, $context->id);
                }
                break;
            case 2:
                foreach ($studentroles as $studentrole) {
        ...
        switch ($forum->assesspublic) {
            case 0:
                ...
                foreach ($teacherroles as $teacherrole) {
                    assign_capability('mod/forumanon:viewanyrating', CAP_ALLOW, $teacherrole->id, $context->id);
                }
                break;
            case 1:
                foreach ($studentroles as $studentrole) {
                    assign_capability('mod/forumanon:viewanyrating', CAP_ALLOW, $studentrole->id, $context->id);
                }
                foreach ($teacherroles as $teacherrole) {
                    assign_capability('mod/forumanon:viewanyrating', CAP_ALLOW, $teacherrole->id, $context->id);
                }
 ...
 * @return array
 */
function forumanon_get_forum_types() {
    return array ('general'  => get_string('generalforum', 'forumanon'),
                  'eachuser' => get_string('eachuserforum', 'forumanon'),
                  'single'   => get_string('singleforum', 'forumanon'),
                  'qanda'    => get_string('qandaforum', 'forumanon'),
                  'blog'     => get_string('blogforum', 'forumanon'));
}
 ...
 * @return array
 */
function forumanon_get_forum_types_all() {
    return array ('news'     => get_string('namenews','forumanon'),
                  'social'   => get_string('namesocial','forumanon'),
                  'general'  => get_string('generalforum', 'forumanon'),
                  'eachuser' => get_string('eachuserforum', 'forumanon'),
                  'single'   => get_string('singleforum', 'forumanon'),
                  'qanda'    => get_string('qandaforum', 'forumanon'),
                  'blog'     => get_string('blogforum', 'forumanon'));
 ...
 * @return array
 */
function forumanon_get_open_modes() {
    return array ('2' => get_string('openmode2', 'forumanon'),
                  '1' => get_string('openmode1', 'forumanon'),
                  '0' => get_string('openmode0', 'forumanon') );
}
 ...
 * @return array
 */
function forumanon_get_extra_capabilities() {
 ...
 */
/*************************************************
function forumanon_extend_navigation($navref, $course, $module, $cm) {
    global $CFG, $OUTPUT, $USER;

    $limit = 5;

    $discussions = forumanon_get_discussions($cm,"d.timemodified DESC", false, -1, $limit);
    $discussioncount = forumanon_get_discussions_count($cm);
    if (!is_array($discussions) || count($discussions)==0) {
        return;
    }
    $discussionnode = $navref->add(get_string('discussions', 'forumanon').' ('.$discussioncount.')');
    ...
    foreach ($discussions as $discussion) {
        ...
        $url = new moodle_url('/mod/forumanon/discuss.php', array('d'=>$discussion->discussion));
    ...
    if ($discussioncount > count($discussions)) {
        if (!empty($navref->action)) {
            ...
            $url = new moodle_url('/mod/forumanon/view.php', array('id'=>$cm->id));
        }
        $discussionnode->add(get_string('viewalldiscussions', 'forumanon'), $url, navigation_node::TYPE_SETTING, null, null, $icon);
    }
    ...
    forumanon_get_recent_mod_activity($recentposts, $index, $lastlogin, $course->id, $cm->id);
    ...
    if (is_array($recentposts) && count($recentposts)>0) {
        ...
        foreach ($recentposts as $post) {
            $icon = new pix_icon('i/feedback', '');
            $url = new moodle_url('/mod/forumanon/discuss.php', array('d'=>$post->content->discussion));
 ...
 */
function forumanon_extend_settings_navigation(settings_navigation $settingsnav, navigation_node $forumnode) {
    global $USER, $PAGE, $CFG, $DB, $OUTPUT;

    $forumobject = $DB->get_record("forumanon", array("id" => $PAGE->cm->instance));
    ...
    if ($cansubscribe) {
        if (forumanon_is_subscribed($USER->id, $forumobject)) {
            $linktext = get_string('unsubscribe', 'forumanon');
        } else {
            $linktext = get_string('subscribe', 'forumanon');
        }
        $url = new moodle_url('/mod/forumanon/subscribe.php', array('id'=>$forumobject->id, 'sesskey'=>sesskey()));
        $forumnode->add($linktext, $url, navigation_node::TYPE_SETTING);
    }

    if (has_capability('mod/forumanon:viewsubscribers', $PAGE->cm->context)){
        $url = new moodle_url('/mod/forumanon/subscribers.php', array('id'=>$forumobject->id));
        $forumnode->add(get_string('showsubscribers', 'forumanon'), $url, navigation_node::TYPE_SETTING);
    }
    ...
    if ($enrolled && forumanon_tp_can_track_forums($forumobject)) { // keep tracking info for users with suspended enrolments
        if ($forumobject->trackingtype != FORUMANON_TRACKING_OPTIONAL) {
            ...
            if (forumanon_tp_is_tracked($forumobject)) {
                $linktext = get_string('notrackforum', 'forumanon');
            } else {
                $linktext = get_string('trackforum', 'forumanon');
            }
            $url = new moodle_url('/mod/forumanon/settracking.php', array('id'=>$forumobject->id));
            $forumnode->add($linktext, $url, navigation_node::TYPE_SETTING);
        }
    }

    if ($enrolled && !empty($CFG->enablerssfeeds) && !empty($CFG->forumanon_enablerssfeeds) && $forumobject->rsstype && $forumobject->rssarticles) {
        ...
        if ($forumobject->rsstype == 1) {
            $string = get_string('rsssubscriberssdiscussions','forumanon');
        } else {
            $string = get_string('rsssubscriberssposts','forumanon');
        }
        if (!isloggedin()) {
            $userid = 0;
        } else {
        ...
        $url = new moodle_url(rss_get_url($PAGE->cm->context->id, $userid, "mod_forumanon", $forumobject->id));
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class forumanon_subscriber_selector_base extends user_selector_base {
...
class forumanon_potential_subscriber_selector extends forumanon_subscriber_selector_base {
     ...
     * determined in {@see forumanon_get_potential_subscribers()}
     *
     * @param string $search
     * @return array
     */
    public function find_users($search) {
        ...
        $availableusers = forumanon_get_potential_subscribers($this->context, $this->currentgroup, $this->required_fields_sql('u'), 'u.firstname ASC, u.lastname ASC');
        ...
        if ($this->forcesubscribed) {
            return array(get_string("existingsubscribers", 'forumanon') => $availableusers);
        } else {
            return array(get_string("potentialsubscribers", 'forumanon') => $availableusers);
        }
    }
...
class forumanon_existing_subscriber_selector extends forumanon_subscriber_selector_base {
    ...
    public function find_users($search) {
        ...
        return array(get_string("existingsubscribers", 'forumanon') => $subscribers);
    }
 ...
 */
function forumanon_cm_info_view(cm_info $cm) {
    global $CFG;
    ...
    if (!isset($initialised)) {
        if ($usetracking = forumanon_tp_can_track_forums()) {
            $strunreadpostsone = get_string('unreadpostsone', 'forumanon');
        }
        $initialised = true;
    }

    if ($usetracking) {
        if ($unread = forumanon_tp_count_forum_unread_posts($cm, $cm->get_course())) {
            $out = '<span class="unread"> <a href="' . $cm->get_url() . '">';
            if ($unread == 1) {
                $out .= $strunreadpostsone;
            } else {
                $out .= get_string('unreadpostsnumber', 'forumanon', $unread);
            }
            $out .= '</a></span>';
