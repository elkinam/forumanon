 ...
 */
function forumanon_rss_get_feed($context, $args) {
    global $CFG, $DB;

    $status = true;

    //are RSS feeds enabled?
    if (empty($CFG->forumanon_enablerssfeeds)) {
        debugging('DISABLED (module configuration)');
        return null;
    }

    $forumid  = clean_param($args[3], PARAM_INT);
    $cm = get_coursemodule_from_instance('forumanon', $forumid, 0, false, MUST_EXIST);
    if ($cm) {
        $modcontext = get_context_instance(CONTEXT_MODULE, $cm->id);

        //context id from db should match the submitted one
        if ($context->id != $modcontext->id || !has_capability('mod/forumanon:viewdiscussion', $modcontext)) {
            return null;
        }
    }

    $forum = $DB->get_record('forumanon', array('id' => $forumid), '*', MUST_EXIST);
    if (!rss_enabled_for_mod('forumanon', $forum)) {
        return null;
    }

    //the sql that will retreive the data for the feed and be hashed to get the cache filename
    $sql = forumanon_rss_get_sql($forum, $cm);

    //hash the sql to get the cache file name
    $filename = rss_get_file_name($forum, $sql);
    $cachedfilepath = rss_get_file_full_name('mod_forumanon', $filename);
    ...
    if ( $dontrecheckcutoff > $cachedfilelastmodified && forumanon_rss_newstuff($forum, $cm, $cachedfilelastmodified)) {
        //need to regenerate the cached version
        $result = forumanon_rss_feed_contents($forum, $sql);
        if (!empty($result)) {
            $status = rss_save_file('mod_forumanon',$filename,$result);
        }
    }
 ...
 * @return void
 */
function forumanon_rss_delete_file($forum) {
    rss_delete_file('mod_forumanon', $forum);
}
 ...
 * @return bool
 */
function forumanon_rss_newstuff($forum, $cm, $time) {
    global $DB;

    $sql = forumanon_rss_get_sql($forum, $cm, $time);

    $recs = $DB->get_records_sql($sql, null, 0, 1);//limit of 1. If we get even 1 back we have new stuff
    return ($recs && !empty($recs));
}

function forumanon_rss_get_sql($forum, $cm, $time=0) {
    $sql = null;

    if (!empty($forum->rsstype)) {
        if ($forum->rsstype == 1) {    //Discussion RSS
            $sql = forumanon_rss_feed_discussions_sql($forum, $cm, $time);
        } else {                //Post RSS
            $sql = forumanon_rss_feed_posts_sql($forum, $cm, $time);
        }
    }
...
function forumanon_rss_feed_discussions_sql($forum, $cm, $newsince=0) {
    global $CFG, $DB, $USER;
    ...
    $groupselect = forumanon_rss_get_group_sql($cm, $groupmode, $currentgroup, $modcontext);
...
function forumanon_rss_feed_posts_sql($forum, $cm, $newsince=0) {
    $modcontext = get_context_instance(CONTEXT_MODULE, $cm->id);
...
function forumanon_rss_get_group_sql($cm, $groupmode, $currentgroup, $modcontext=null) {
    $groupselect = '';
 ...
 * @param bool
 */
function forumanon_rss_feed_contents($forum, $sql) {
    global $CFG, $DB;
    ...
    foreach ($recs as $rec) {
            ...
            if ($isdiscussion) {
                $item->link = $CFG->wwwroot."/mod/forumanon/discuss.php?d=".$rec->discussionid;
            } else {
                $item->link = $CFG->wwwroot."/mod/forumanon/discuss.php?d=".$rec->discussionid."&parent=".$rec->postid;
            }
            ...
            /*if (!$isdiscussion) {
                $post_file_area_name = str_replace('//', '/', "$forum->course/$CFG->moddata/forumanon/$forum->id/$rec->postid");
    ...
    if (!empty($items)) {
        ...
        $header = rss_standard_header(strip_tags(format_string($forum->name,true)),
                                      $CFG->wwwroot."/mod/forumanon/view.php?f=".$forum->id,
