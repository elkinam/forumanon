...
if (empty($search)) {   // Check the other parameters instead
    ...
    if (!empty($user)) {
        $search .= ' '.forumanon_clean_search_terms($user, 'user:');
    }
    if (!empty($subject)) {
        $search .= ' '.forumanon_clean_search_terms($subject, 'subject:');
    }
    if (!empty($fullwords)) {
        $search .= ' '.forumanon_clean_search_terms($fullwords, '+');
    }
    if (!empty($notwords)) {
        $search .= ' '.forumanon_clean_search_terms($notwords, '-');
    }
    if (!empty($phrase)) {
        $search .= ' "'.$phrase.'"';
    }
...
if ($search) {
    $search = forumanon_clean_search_terms($search);
}
...
add_to_log($course->id, "forumanon", "search", "search.php?id=$course->id&amp;search=".urlencode($search), $search);

$strforums = get_string("modulenameplural", "forumanon");
$strsearch = get_string("search", "forumanon");
$strsearchresults = get_string("searchresults", "forumanon");
$strpage = get_string("page");

if (!$search || $showform) {

    $PAGE->navbar->add($strforums, new moodle_url('/mod/forumanon/index.php', array('id'=>$course->id)));
    $PAGE->navbar->add(get_string('advancedsearch', 'forumanon'));

    $PAGE->set_title($strsearch);
    $PAGE->set_heading($course->fullname);
    echo $OUTPUT->header();

    forumanon_print_big_search_form($course);
    echo $OUTPUT->footer();
    exit;
}
...
$searchform = forumanon_search_form($course, $search);

$PAGE->navbar->add($strsearch, new moodle_url('/mod/forumanon/search.php', array('id'=>$course->id)));
$PAGE->navbar->add(s($search, true));
if (!$posts = forumanon_search_posts($searchterms, $course->id, $page*$perpage, $perpage, $totalcount)) {
    $PAGE->set_title($strsearchresults);
    $PAGE->set_heading($course->fullname);
    echo $OUTPUT->header();
    echo $OUTPUT->heading(get_string("nopostscontaining", "forumanon", $search));
...
echo '<a href="search.php?id='.$course->id.
                         ...
                         '&amp;datefrom='.$datefrom.
                         '&amp;showform=1'.
                         '">'.get_string('advancedsearch','forumanon').'...</a>';
echo '</div>';
...
foreach ($posts as $post) {
    ...
    if (! $discussion = $DB->get_record('forumanon_discussions', array('id' => $post->discussion))) {
        print_error('invaliddiscussionid', 'forumanon');
    }
    ...
    // the start of the message text. However, do not do the highlighting here. forumanon_print_post
    ...
    if ($missing_terms) {
        $strmissingsearchterms = get_string('missingsearchterms','forumanon');
    ...
    $fulllink = "<a href=\"discuss.php?d=$post->discussion#p$post->id\">".get_string("postincontext", "forumanon")."</a>";
 ...
 */
function forumanon_print_big_search_form($course) {
    global $CFG, $DB, $words, $subject, $phrase, $user, $userid, $fullwords, $notwords, $datefrom, $dateto, $PAGE, $OUTPUT;

    echo $OUTPUT->box(get_string('searchforumintro', 'forumanon'), 'searchbox boxaligncenter', 'intro');

    echo $OUTPUT->box_start('generalbox boxaligncenter');

    echo html_writer::script('', $CFG->wwwroot.'/mod/forumanon/forum.js');

    echo '<form id="searchform" action="search.php" method="get">';
    echo '<table cellpadding="10" class="searchbox" id="form">';

    echo '<tr>';
    echo '<td class="c0"><label for="words">'.get_string('searchwords', 'forumanon').'</label>';
    ...
    echo '<tr>';
    echo '<td class="c0"><label for="phrase">'.get_string('searchphrase', 'forumanon').'</label></td>';
    echo '<td class="c1"><input type="text" size="35" name="phrase" id="phrase" value="'.s($phrase, true).'" alt="" /></td>';
    echo '</tr>';

    echo '<tr>';
    echo '<td class="c0"><label for="notwords">'.get_string('searchnotwords', 'forumanon').'</label></td>';
    ...
    if ($DB->get_dbfamily() == 'mysql' || $DB->get_dbfamily() == 'postgres') {
        echo '<tr>';
        echo '<td class="c0"><label for="fullwords">'.get_string('searchfullwords', 'forumanon').'</label></td>';
        echo '<td class="c1"><input type="text" size="35" name="fullwords" id="fullwords" value="'.s($fullwords, true).'" alt="" /></td>';
        echo '</tr>';
    }

    echo '<tr>';
    echo '<td class="c0">'.get_string('searchdatefrom', 'forumanon').'</td>';
    echo '<td class="c1">';
    if (empty($datefrom)) {
        $datefromchecked = '';
    ...
    echo '<input name="timefromrestrict" type="checkbox" value="1" alt="'.get_string('searchdatefrom', 'forumanon').'" onclick="return lockoptions(\'searchform\', \'timefromrestrict\', timefromitems)" '.  $datefromchecked . ' /> ';
    ...
    echo '<tr>';
    echo '<td class="c0">'.get_string('searchdateto', 'forumanon').'</td>';
    echo '<td class="c1">';
    if (empty($dateto)) {
        $datetochecked = '';
        $dateto = time()+3600;
    ...
    echo '<input name="timetorestrict" type="checkbox" value="1" alt="'.get_string('searchdateto', 'forumanon').'" onclick="return lockoptions(\'searchform\', \'timetorestrict\', timetoitems)" ' .$datetochecked. ' /> ';
    ...
    echo '<tr>';
    echo '<td class="c0"><label for="menuforumid">'.get_string('searchwhichforums', 'forumanon').'</label></td>';
    echo '<td class="c1">';
    echo html_writer::select(forumanon_menu_list($course), 'forumid', '', array(''=>get_string('allforums', 'forumanon')));
    echo '</td>';
    echo '</tr>';

    echo '<tr>';
    echo '<td class="c0"><label for="subject">'.get_string('searchsubject', 'forumanon').'</label></td>';
    echo '<td class="c1"><input type="text" size="35" name="subject" id="subject" value="'.s($subject, true).'" alt="" /></td>';
    echo '</tr>';

    echo '<tr>';
    echo '<td class="c0"><label for="user">'.get_string('searchuser', 'forumanon').'</label></td>';
    ...
    echo '<input type="submit" value="'.get_string('searchforums', 'forumanon').'" alt="" /></td>';
    echo '</tr>';
 ...
 */
function forumanon_clean_search_terms($words, $prefix='') {
 ...
 * @todo Document this function
 */
function forumanon_menu_list($course)  {
    ...
    foreach ($modinfo->instances['forumanon'] as $cm) {
        if (!$cm->uservisible) {
            continue;
        }
